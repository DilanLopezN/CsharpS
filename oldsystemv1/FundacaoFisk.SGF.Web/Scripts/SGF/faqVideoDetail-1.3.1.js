...
function montarMetodosVideoFaq() {
	...
	require([
		...
		ready(function () {
			...
			try {
				...
				var cd_faq = urlParams.get("cd_faq");
				
				

				xhr.get({
					url: Endereco() + "/api/coordenacao/obterFaqPorID",
					preventCache: true,
					content: {
					    cd_faq: cd_faq
					},
				    ...
				    if (hasValue(dataVideo)) {
						...
						var caminhoVideo = Endereco() + "/Videos/" + video.retorno.no_video_faq;

						dom.byId("nomeVid").innerHTML = "Pergunta: " + video.retorno.dc_faq_pergunta;
