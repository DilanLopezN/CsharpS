...
function keepValues(value, grid, ehLink) {
    try{
        ...
        if (hasValue(value)) {
            ...
            if(hasValue(value.cd_escola))
                dojo.byId("cd_escola").value = value.cd_escola;
    ...
    require([
        ...
        ready(function () {
            try {
                ...
                var myStore = Cache(
                    JsonRest({
                        target: Endereco() + "/api/coordenacao/getModeloProgramacaoCursoSearch?cdCurso=" + cdCurso + "&cdDuracao=" + cdDuracao,
                        handleAs: "json",
                ...
                var acaoEditar = new MenuItem({
                    label: "Editar",
                    onClick: function () { eventoEditar(gridProgramacao.itensSelecionados); }
                });
                menu.addChild(acaoEditar);

                var acaoRemover = new MenuItem({
                    label: "Excluir",
                    onClick: function () { eventoRemover(gridProgramacao.itensSelecionados, 'DeletarProgramacao(itensSelecionados)'); }
                });
                ...
                if (hasValue(dijit.byId("menuManual"))) {
                    dijit.byId("menuManual").on("click",
                        function(e) {
                            abrePopUp(Endereco() + '/Content/manual/manual.htm#_Toc423101886', '765px', '771px');
                        });
                }
                xhr.get({
...
function loadGridProgramacao(cdCurso, cdDuracao, pFuncao) {
    require([
        ...
        xhr.get({
            url: Endereco() + "/api/coordenacao/getModeloCursoProgramacao?cdCurso=" + cdCurso + "&cdDuracao=" + cdDuracao,
            handleAs: "json",
            preventCache: true,
            headers: { "Accept": "application/json", "Authorization": Token() },
            ...
            try{
                ...
                new Button({
                    ...
                    onClick: function () {
                        ...
                        var cdDuracao = dijit.byId("cbDuracao").get("value") == null || dijit.byId("cbDuracao").get("value") == "" ? 0 : dijit.byId("cbDuracao").get("value");
                            xhr.get({
                                url: Endereco() + "/api/coordenacao/getUrlRelatorioModeloProgramacaoCursoSearch?" + getStrGridParameters('gridProgramacao') + "cdCurso=" + cdCurso + "&cdDuracao=" + cdDuracao,
                                preventCache: true,
                                handleAs: "json",
                                headers: { "Accept": "application/json", "Content-Type": "application/json", "Authorization": Token() }
                            }).then(function (data) {
...
function IncluirProgramacao() {
    try {
        ...
        require(["dojo/dom",  "dojo/request/xhr", "dojox/json/ref"], function (dom, xhr, ref) {
            ...
            showCarregando();
            xhr.post(Endereco() + "/api/coordenacao/postModeloProgramacaoCurso", {
                headers: { "Accept": "application/json", "Content-Type": "application/json", "Authorization": Token() },
                handleAs: "json",
                data: ref.toJson({
    ...
    try {
        ...
        if (descBranco <= 0) {
            ...
            require(["dojo/dom", "dojo/_base/xhr", "dojox/json/ref"], function (dom, xhr, ref) {
                xhr.post({
                    url: Endereco() + "/api/coordenacao/postAlterarModeloProgramacaoCurso",
                    headers: { "Accept": "application/json", "Content-Type": "application/json", "Authorization": Token() },
                    handleAs: "json",
                    postData: ref.toJson({
                        cd_escola: dojo.byId("cd_escola").value,
...
function DeletarProgramacao(itensSelecionados) {
    try {
        ...
        require(["dojo/dom", "dojo/_base/xhr", "dojox/json/ref"], function (dom, xhr, ref) {
            ...
            xhr.post({
                url: Endereco() + "/api/coordenacao/PostDeleteModeloProgramacaoCurso",
...
function atualiza(i) {
    try{
        var gridProg = dijit.byId('gridProg');
        if (hasValue(gridProg.getRowNode(i))) {
