...
function montarMetodosBaixaAutomaticaCheque() {
    ...
    require([
        ...
        ready(function () {
            //*** Cria a grade de baixa cheque **\\
            dijit.byId('tabContainer').resize();
            var data = [];

            //GRID BAIXACHEQUE
//            var gridBaixaCheque = new EnhancedGrid({
...
//                    },
//                    { name: "Cheque", field: "dc_num_documento_titulo", width: "25%", styles: "min-width:80px;" },
//                    { name: "Pessoa", field: "no_pessoa", width: "40%", styles: "min-width:70px;" },
//                    { name: "Emissor", field: "no_emissor", width: "40%", styles: "min-width:70px;" },
...
//                //    return colIndex !=0 && field != 'selecionadoTitulo';
//                //},
...
//                "gridBaixaCheque");
//
//            gridBaixaCheque.canSort = function (col) { return Math.abs(col) != 1 };
//            //gridBaixaCheque.canSort = function () { return true };
//            gridBaixaCheque.startup();

//            require(["dojo/aspect"], function (aspect) {
//                aspect.after(gridBaixaCheque, "_onFetchComplete", function () {
//                    // Configura o check de todos:
//                    if (dojo.byId('selecionaTodosTitulo').type == 'text')
//                        setTimeout("configuraCheckBox(false, 'cd_titulo', 'selecionadoTitulo', -1, 'selecionaTodosTitulo', 'selecionaTodosTitulo', 'gridBaixaCheque')", gridBaixaCheque.rowsPerPage * 3);
//                });
//            });
...
//            montarGridGeradas();
            ...
            //    label: "Todos Itens",
            //    onClick: function () { buscarTodosItens(gridBaixaCheque, 'todosItensBaixaCheque', ['pesquisarTitulos']); getTitulosForBaixaAutomatica(); }
            //});
            ...
            //    onClick: function () {
            //        buscarItensSelecionados('gridBaixaCheque', 'selecionadoTitulo', 'cd_titulo', 'selecionaTodosTitulo', ['pesquisarTitulos'], 'todosItensBaixaCheque');
            ...
            //    label: "Todos Itens",
            //    name: "todosItensBaixaCheque",
            //    dropDown: menu,
            //    id: "todosItensBaixaCheque"
            //});
            ...
            //    label: "Gerar Baixa",
            //    onClick: function () {
            //        gerarBaixaAutomatica(Memory, ObjectStore, dijit.byId("gridBaixaCheque"));
            //    }
            //});
            ...
            //    name: "acoesRelacionadasBaixaCheque",
            //    dropDown: menu,
            //    id: "acoesRelacionadasBaixaCheque"
            //});
            //dom.byId("linkAcoesBaixaCheque").appendChild(button.domNode);
...
//                    }
            ...
            var acaoDetalhe = new MenuItem({
                ...
                onClick: function () {
                    ...
                    if (grid.itensSelecionados != null && grid.itensSelecionados.length == 0 || grid.itensSelecionados == null) {
                        ...
                        getBaixasEfetuadasForBaixaAutomaticaCheque(grid.itensSelecionados[0]);
                    }
                }
            });
            ...
            //if (dojo.byId('selecionaTodosTitulo').type == 'text')
            //    setTimeout(
            //        "configuraCheckBox(false, 'cd_titulo', 'selecionadoTitulo', -1, 'selecionaTodosTitulo', 'selecionaTodosTitulo', 'gridBaixaCheque')",
            //        gridBaixaCheque.rowsPerPage * 3);
    ...
    //    try {
    //        var grid = dijit.byId("gridBaixaCheque");
    //        grid.store.save();
    ...
    function getLocalMovtoBaixaAut() {
        dojo.xhr.get({
            url: Endereco() + "/api/financeiro/getLocalMovtoBaixaAutomaticaCheque",
            preventCache: true,
            handleAs: "json",
            headers: { "Accept": "application/json", "Content-Type": "application/json", "Authorization": Token() }
        }).then(function (data) {
            try {
                ...
                loadSelect(response.retorno, "lista_localmovt_cheque", 'cd_local_movto', 'no_local_movto');
            } catch (e) {
                postGerarLog(e);
            }
    ...
    function getLocaisMovtoBaixaAutById(cd_local) {

        if (hasValue(cd_local)) {
            dojo.xhr.get({
                url: Endereco() + "/api/financeiro/getAllLocalMovtoCheque?cd_local_banco=" + cd_local,
                preventCache: true,
                handleAs: "json",
                headers: { "Accept": "application/json", "Content-Type": "application/json", "Authorization": Token() }
            }).then(function (data) {
                try {
                    ...
                    if (hasValue(response.retorno)) {
                        dijit.byId('lista_localmovt_cheque').setAttribute('disabled', false);
                        loadSelect(response.retorno, "lista_localmovt_cheque", 'cd_local_movto', 'no_local_movto');
                    } else {
                        dijit.byId('lista_localmovt_cheque').setAttribute('disabled', true);
                    }
                } catch (e) {
                    postGerarLog(e);
                }
            ...
            dijit.byId("lista_localmovt_cheque").reset();
            dijit.byId('lista_localmovt_cheque').setAttribute('disabled', true);
        }
    }
    ...
    //                dijit.byId("lista_localmovt_cheque").validate() == false)) {
    //                return false;
    //            } else {
    ...
    //                    handleAs: "json",
    //                    postData: dojox.json.ref.toJson({
    //                        cd_local_movto: dijit.byId("lista_localmovt_cheque").get("value"),
    ...
    //                        id_tipo: 2,
    //                        cds_titulos: titulos,
    //                        id_trocar_local: dijit.byId("ckTroca").checked
    //                    })
    //                }).then(function (data) {
    //                    try {
    ...
    //                        getTitulosForBaixaAutomatica();
    //                        //montarCadastroMotivoMatricula();
    //                        //gridBaixaCheque.setStore(new dojo.data.ObjectStore({ objectStore: new dojo.store.Memory({ data: data }) }));
    //                    } catch (e) {
    //                        postGerarLog(e);
    //                    }
    //                    showCarregando();
        ...
        try {
            ...
            if ((dijit.byId("formFiltros").validate() == false ||
                ...
                require([
                    ...
                    var gridGeradas = new EnhancedGrid({
                        ...
                        structure: [
                            ...
                            { name: "Trocar", field: "id_trocar_local", width: "30px", styles: "min-width:30px;text-align:center;", formatter: trocaLocalFormatter },
                ...
                require([
                    ...
                    //    registry.byId("gridGeradas").destroyRecursive();
                    //}
                    var local = hasValue(dojo.byId("lista_localmovt_cheque").value) ? dijit.byId("lista_localmovt_cheque").get("value") : 0;
                    var storeGeradas = Cache(
                                JsonRest({
                                    target: Endereco() + "/api/financeiro/listarBaixaAutomaticasEfetuadasCheque?cd_local_movto=" + local + 
				    ...
				    "&dh_baixa_automatica=" + montarDataHoraBaixaAutomatica() +
				    "&id_tipo=" + 2 + "&id_trocar_local=" + true, //dijit.byId("ckTroca").checked,
                                    handleAs: "json",
                                    headers: { "Accept": "application/json", "Content-Type": "application/json", "Authorization": Token() },
                    ...
                    gridGeradas.on("RowDblClick", function (evt) {
                        try {
                            ...
                            if (hasValue(item)) {
                                ...
                                var pane = dijit.byId("tgBaixas");
                                tabs.selectChild(pane);
                                getBaixasEfetuadasForBaixaAutomaticaCheque(item);
                            }
                        }
                        catch (e) {
                            postGerarLog(e);
    ...
    function getBaixasEfetuadasForBaixaAutomaticaCheque(itensSelecionados) {
        try {
                ...
                function (dom,
                    ...
                    ready) {
                        ...
                        var storeBaixas = Cache(
                            JsonRest({
                                    target: Endereco() +
                                        "/api/financeiro/getBaixasEfetuadasForBaixaAutomaticaCheque?cd_baixa_automatica="+ itensSelecionados.cd_baixa_automatica +
					                    "&cd_local_movto=" + local + //dijit.byId("lista_localmovt_cheque").get("value") +
                                        "&dt_inicial=" + montarDataBaixaAutomatica(dijit.byId("dtaInicial").get("value")) +
                                        "&dt_final=" + montarDataBaixaAutomatica(dijit.byId("dtaFinal").get("value")) +
                                        "&dh_baixa_automatica=" +  montarDataHoraBaixaAutomatica() +
                                        "&id_tipo=" + 2 +
                                        "&id_trocar_local=" + true,
                                    handleAs: "json",
                                    headers: {
                        ...
                        var gridBaixas = new EnhancedGrid({
                            ...
                            structure: [
                                ...
                                { name: "Emissor", field: "no_emitente_cheque", width: "25%", styles: "min-width:80px;" },
                                { name: "Pessoa", field: "no_pessoa", width: "30%", styles: "min-width:70px;" },
                                { name: "Titulo", field: "nm_titulo", width: "70px", styles: "min-width:70px;text-align:center;" },
                                { name: "Parcela", field: "nm_parcela_titulo", width: "70px", styles: "min-width:70px;text-align:center;" },
                                { name: "Vencimento", field: "dt_vcto_titulo", width: "70px", styles: "min-width:70px;", formatter: formatDataTitulo },
...
//                var gridBaixaCheque = dijit.byId("gridBaixaCheque");
//                showCarregando();
//                dojo.xhr.post({
//                    url: Endereco() + "/api/financeiro/getTitulosForBaixaAutomaticaCheque",
//                    headers: { "Accept": "application/json", "Content-Type": "application/json", "Authorization": Token() },
//                    handleAs: "json",
//                    postData: dojox.json.ref.toJson({
//                        cd_local_movto_cheque: dijit.byId("lista_localmovt_cheque").get("value"),
//                        dtInicial: dijit.byId("dtaInicial").get("value"),
//                        dtFinal: dijit.byId("dtaFinal").get("value"),
//                        troca_local: dijit.byId("ckTroca").checked
//                    })
//                }).then(function (data) {
//                    try {
//                        //data = jQuery.parseJSON(data).retorno;
...
function atualizaDetalheBaixaCheque(itensSelecionados) {
    require([
        "dojo/store/JsonRest",
        "dojo/data/ObjectStore",
        "dojo/store/Cache",
        ...
        try {
            var myStore = Cache(
                JsonRest({
                    target: Endereco() + "/api/financeiro/getBaixasEfetuadasForBaixaAutomaticaCheque?cd_baixa_automatica=" + itensSelecionados.cd_baixa_automatica +
                        "&cd_local_movto=" + dijit.byId("lista_localmovt_cheque").get("value") +
                        "&dt_inicial=" + montarDataBaixaAutomatica(dijit.byId("dtaInicial").get("value")) +
                        "&dt_final=" + montarDataBaixaAutomatica(dijit.byId("dtaFinal").get("value")) +
                        "&dh_baixa_automatica=" + montarDataHoraBaixaAutomatica() + "&id_tipo=" + 2 +
                        "&id_trocar_local=" + true,
                        handleAs: "json",
                        headers: { "Accept": "application/json", "Content-Type": "application/json", "Authorization": Token() },
                        idProperty: "cd_baixa_automatica"
    ...
    try {
        ...
        if (!hasValue(tab)) // Clicou na borda da aba:
            ...
            if (tab.innerText == 'Geradas') {
                ...
                if (!tab.innerText.startsWith('Baixas')) {
                    dijit.byId('tgBaixas').set('disabled', true);
                    dijit.byId('tgBaixas').set('title', 'Baixas');
                dijit.byId("pesquisarTitulos").set('disabled', true);
                }
...
function setarEventosBotoesPrincipaisCadTransacao(xhr, on) {
    try {
        ...
        dijit.byId("alterarBaixa").on("click", function () {
            alterarBaixaTelaCheque(xhr, dojox.json.ref, dojo.ready, dojo.store.Memory, dijit.form.FilteringSelect, dojo.on);
        });
...
function alterarBaixaTelaCheque(xhr, ref, ready, Memory, FilteringSelect, on) {
    try {
            ...
            try {
               // apresentaMensagem('apresentadorMensagem', data);
                ...
                if (grid.itensSelecionados != null && grid.itensSelecionados.length == 0 || grid.itensSelecionados == null) {
                    ...
                    atualizaDetalheBaixaCheque(grid.itensSelecionados[0]);
                    dijit.byId("cadBaixaFinanceira").hide();
                }
...
function deletarTransFinan(xhr, ref) {
    try {
        ...
        if (grid.itensSelecionados != null && grid.itensSelecionados.length == 0 || grid.itensSelecionados == null) {
            ...
            atualizaDetalheBaixaCheque(gridGeradas.itensSelecionados[0]);
            dijit.byId("cadBaixaFinanceira").hide();
        }
