...
define([
	...
	var _Grid = declare('dojox.grid._Grid',
		...
		showMessage: function(message){
			//Correção do dojo - by Eduardo Chagas:
...
=====*/
	...
	_onFetchComplete: function(items, req){
        ...
        // Correção do dojo, para quando excluir todos os itens da ultima página, ele ir para penúltima - Eduardo Chagas:
		if (!items || items.length <= 0)
			if (typeof this['currentPage'] === "function" && this.currentPage() > 1)
	...
	onFetchError: function(err, req){
	    // Eduardo - Realiza o tratamento de exceção para acesso negado ou  sessão expirada:
	    if (err != undefined && err.response != undefined && err.response.data != undefined) {
	        var dataErro = err.response.data;

	        try{
	            dataErro = eval(dataErro);
	        }
	        catch (e) {
	        }

	        dataErro = jQuery.parseJSON(dataErro);
	        var mensagensErro = dataErro.erro != undefined ? dataErro.erro.MensagensWeb : (dataErro.MensagensWeb != undefined ? dataErro.MensagensWeb : null);

	        if (hasValue(mensagensErro))
	            for (var i = 0; i < mensagensErro.length; i++) {
	                var erro = mensagensErro[i];

	                if (erro.tipoMensagem == 1) {
	                    this.errorMessage = "<span class='dojoxGridError'>" + erro.mensagem + "</span>";
	                    this.showMessage(this.errorMessage);
	                }
	            }
	    }
