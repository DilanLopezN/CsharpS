...
[assembly: WebResource("BoletoNet.Imagens.238.jpg", "image/jpg")]

namespace BoletoNet
{
    public class Banco_DinariPay : AbstractBanco, IBanco
    {
        private string _dacNossoNumero = string.Empty;
        private int _dacBoleto = 0;

        /// <summary>
        /// Classe responsavel em criar os campos do Banco Banco_DinariPay.
        /// </summary>
        internal Banco_DinariPay()
        {
            this.Codigo = 238;
            this.Digito = "2";
            this.Nome = "DinariPay";
        ...
        {
            return Mod11DinariPay(boleto.Carteira + boleto.NossoNumero, 7);
        }
        ...
        private string Mod11DinariPay(string seq, int b)
        {
        ...
        public string GerarDetalheRemessaCNAB400(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                _detalhe += Mod11DinariPay(boleto.Carteira + Utils.FitStringLength(boleto.NossoNumero, 11, 11, '0', 0, true, true, true), 7); // Digito de Auto Conferencia do Nosso Número (01)
                ...
                */
                ...
                {
                    switch ((EnumInstrucoes_DinariPay)instrucao.Codigo)
                    {
        ...
        public string GerarMensagemVariavelRemessaCNAB400(Boleto boleto, ref int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                //_detalhe += new string(' ', 13);
                //_detalhe += GerarRegistroDetalhe2(boleto, numeroRegistro);


                //_detalhe += new string(' ', 320);                       // 002 a 321 Mensagens 1,2,3,4

                if (boleto.DataOutrosDescontos == DateTime.MinValue || boleto.OutrosDescontos == 0)    // 322 a 327 Data limite para concessão de Desconto 2
                {
                    _detalhe += "000000"; //Caso nao tenha data de vencimento
                }
                else
                ...
                if (boleto.DataOutrosDescontos2 == DateTime.MinValue || boleto.OutrosDescontos2 == 0)    // 322 a 327 Data limite para concessão de Desconto 2
                {
                    _detalhe += "000000"; // 341 a 346 data limite para concessão de Desconto 3
                }
                else
                {
                    _detalhe += boleto.DataOutrosDescontos2.ToString("ddMMyy");
                }

                // 347 a 359 Valor do Desconto 3
                _detalhe += Utils.FitStringLength(boleto.OutrosDescontos2.ToString("0.00").Replace(",", ""), 13, 13, '0', 0, true, true, true);
                _detalhe = Utils.SubstituiCaracteresEspeciais(_detalhe);
        ...
        public string GerarRegistroDetalhe2(Boleto boleto, int numeroRegistro)
        {
            ...
            // 328 a 340 Valor do Desconto 2
            _detalhe += Utils.FitStringLength(boleto.OutrosDescontos.ToString("0.00").Replace(",", ""), 13, 13, '0', 0, true, true, true);
            if (boleto.DataOutrosDescontos2 == DateTime.MinValue)    // 322 a 327 Data limite para concessão de Desconto 2
            {
                _detalhe += "000000"; // 341 a 346 data limite para concessão de Desconto 3
            }
            else
            ...
            // Força o NossoNumero a ter 11 dígitos. Alterado por Luiz Ponce 07/07/2012
            _detalhe += Mod11DinariPay(boleto.Carteira + Utils.FitStringLength(boleto.NossoNumero, 11, 11, '0', 0, true, true, true), 7); // 394 a 394 Digito de Auto Conferencia do Nosso Número (01)
            //Desconto Bonificação por dia (10, N)
