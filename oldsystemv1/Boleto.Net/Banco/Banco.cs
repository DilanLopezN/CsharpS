...
namespace BoletoNet
{
    public class Banco : AbstractBanco, IBanco
    {
        ...
        public override void FormataCodigoCedente(Boleto boleto)
        {
            try
            {
                _IBanco.FormataCodigoCedente(boleto);
            }
            catch (Exception ex)
            {
                throw new Exception("Erro durante a formatação do nosso número.", ex);
        ...
        public override string GerarTrailerRemessa(int numeroRegistro, TipoArquivo tipoArquivo, Cedente cedente, decimal vltitulostotal, int qtdBoletos)
        {
            try
            {
                return _IBanco.GerarTrailerRemessa(numeroRegistro, tipoArquivo, cedente, vltitulostotal, qtdBoletos);
            }
            catch (Exception ex)
            {
                throw new Exception("Erro durante a geração do registro TRAILER do arquivo de REMESSA.", ex);
