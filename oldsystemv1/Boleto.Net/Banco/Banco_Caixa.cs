...
namespace BoletoNet
{
    ...
    internal class Banco_Caixa : AbstractBanco, IBanco
    {
        ...
        public override void FormataCodigoBarra(Boleto boleto)
        {
            ...
            if (boleto.Carteira.Equals("SR") || boleto.Carteira.Equals("RG"))
            {
                ...
                if (boleto.NossoNumero.Length == 17)
                {
                    ...
                    // Posição 26
                    string dvCodigoCedente = "";

                    //Aplica - se somente para códigos de beneficiários cadastrados entre as faixas 000001 e 999999.
                    if ((Convert.ToInt32(boleto.Cedente.Codigo) >= 000001 && (Convert.ToInt32(boleto.Cedente.Codigo) <= 999999)))
                    {
                        codigoCedente = Utils.FormatCode(Convert.ToInt32(boleto.Cedente.Codigo).ToString(), 6);

                        dvCodigoCedente = Mod11Base9(codigoCedente).ToString();

                    }
                    //Aplica - se somente para códigos de beneficiários cadastrados entre na faixa >= 1100000
                    else if ((Convert.ToInt32(boleto.Cedente.Codigo) >= 1100000))
                    {
                        codigoCedente = Utils.FormatCode(Convert.ToInt32(boleto.Cedente.Codigo).ToString(), 6).Substring(0, 6);

                        dvCodigoCedente = boleto.Cedente.Codigo[6].ToString();
                    }
            ...
            else
            {
                ...
                if ((Convert.ToInt32(boleto.Cedente.Codigo) >= 000001 && (Convert.ToInt32(boleto.Cedente.Codigo) <= 999999)))
                {
                    codigoCedente = Utils.FormatCode(boleto.Cedente.Codigo.ToString(), 6);
                ...
                {
                    codigoCedente = Utils.FormatCode(boleto.Cedente.Codigo.ToString(), 6).Substring(0, 6);
        ...
        public override void ValidaBoleto(Boleto boleto)
        {
            ...
            if (Convert.ToInt32(boleto.Cedente.Codigo).ToString().Length > 6 && (Convert.ToInt32(boleto.Cedente.Codigo) < 1100000))
                throw new Exception("O código do cedente deve conter apenas 6 dígitos");

            //Atribui o nome do banco ao local de pagamento
            if (string.IsNullOrEmpty(boleto.LocalPagamento))
                boleto.LocalPagamento = "EM TODA A REDE BANCÁRIA E SEUS CORRESPONDENTES ATÉ O VALOR LIMITE";
        ...
        public string GerarHeaderRemessaCNAB400(int numeroConvenio, Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                if((Convert.ToInt32(cedente.Codigo) >= 000001 && (Convert.ToInt32(cedente.Codigo) <= 999999)))
                {
                    //Para Nº da Versão do Layout esteja preenchido com ‘007’ (aplicável para código de beneficiário até 999999)
                    //Posição => 31 - 37
                    //Posição inicial do Código do Beneficiário => 32
                    //Informar ‘zero’ na posição => 31
                    
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliDireita______, 0031, 006, 0, Convert.ToInt32(cedente.Codigo), '0'));                         //031-036
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0037, 001, 0, " ", ' '));                         //031-036
                }
                else if ((Convert.ToInt32(cedente.Codigo) >= 1100000))
                {
                    //PARA CÓDIGO DE BENEFICIÁRIO A PARTIR DE 1100000
                    //Para Código de Beneficiário iniciado a partir de 1100000, utilizar somente o Nº da Versão do Layout ‘007’, uma vez que serão utilizados todos os campos disponíveis para preenchimento.
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliDireita______, 0031, 007, 0, Convert.ToInt32(cedente.Codigo), '0'));                         //031-036
                }

                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0038, 009, 0, " ", ' '));                                    //037-046
                ...
                if ((Convert.ToInt32(cedente.Codigo) >= 000001 && (Convert.ToInt32(cedente.Codigo) <= 999999)))
                {
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0101, 003, 0, "   ", ' '));
                }
                else if ((Convert.ToInt32(cedente.Codigo) >= 1100000))
                {
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0101, 003, 0, "007", ' '));                                     //101-389
                }

                
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0104, 286, 0, "", ' '));                                     //101-389
        ...
        public string GerarDetalheRemessaCNAB400(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediNumericoSemSeparador_, 0018, 003, 0, 000, '0'));      //018-021

                if ((Convert.ToInt32(boleto.Cedente.Codigo) >= 000001 && (Convert.ToInt32(boleto.Cedente.Codigo) <= 999999)))
                {
                    //Para Nº da Versão do Layout esteja preenchido com ‘007’ (aplicável para código de beneficiário até 999999)
                    //Posição => 21 - 27
                    //Posição inicial do Código do Beneficiário => 22
                    //Informar ‘zero’ na posição => 21
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0021, 001, 0, " ", ' '));                         //021
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliDireita______, 0022, 006, 0, Convert.ToInt32(boleto.Cedente.Codigo), '0'));                         //022-027
                }
                else if ((Convert.ToInt32(boleto.Cedente.Codigo) >= 1100000))
                {
                    //PARA CÓDIGO DE BENEFICIÁRIO A PARTIR DE 1100000
                    //Para Código de Beneficiário iniciado a partir de 1100000, utilizar somente o Nº da Versão do Layout ‘007’, uma vez que serão utilizados todos os campos disponíveis para preenchimento.
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliDireita______, 0021, 007, 0, Convert.ToInt32(boleto.Cedente.Codigo), '0'));                         //021-027
                }

                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediNumericoSemSeparador_, 0028, 001, 0, 2, '0'));                                         //028-028
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediNumericoSemSeparador_, 0029, 001, 0, 0, '0'));                                         //029-029
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediNumericoSemSeparador_, 0030, 002, 0, 00, '0'));                                        //030-031
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0032, 025, 0, boleto.NossoNumero, ' '));                        //032-056
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediNumericoSemSeparador_, 0057, 002, 0, 14, '0'));                                        //057-058
                ...
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0077, 001, 0, " ", ' '));  //077-077

                //Data DataVencimento Juros
                #region DataVencimentoJuros
                string vDataVencimentoJuros= "      ";
                if (!boleto.DataVencimento.Equals(DateTime.MinValue))
                {
                    DateTime dataVencimentoAddOneDay = boleto.DataVencimento.AddDays(1);
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediDataDDMMAA___________, 0078, 006, 0, dataVencimentoAddOneDay, ' '));

                }
                else
                {
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0078, 006, 0, vDataVencimentoJuros, ' '));
                }
                ...
                if (!boleto.DataDesconto.Equals(DateTime.MinValue)) //se tem desconto
                {
                    if (boleto.ValorDesconto > 0)
                    {
                        reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0084, 001, 0, 1, '0'));                              //077-106
                    }
                    else
                    {
                        reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0084, 001, 0, 2, '0'));                              //077-106
                    }
                    
                }
                else
                {
                    reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0084, 001, 0, 0, '0'));                              //077-106
                }


                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0085, 022, 0, string.Empty, ' '));
                ...
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0148, 002, 0, especie, '0'));                                   //148-149
                ...
                //
                #region Instruções
                string vInstrucao1 = "02";
                string vInstrucao2 = "0";
                string vInstrucao3 = "0";
                int prazoProtesto_Devolucao = 25;
                foreach (IInstrucao instrucao in boleto.Instrucoes)
                {
                    switch ((EnumInstrucoes_Caixa)instrucao.Codigo)
                    {
                ...
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0275, 040, 0, Utils.SubstituiCaracteresEspeciais(boleto.Sacado.Endereco.End).ToUpper(), ' '));      //275-314
                ...
                #region Multa
                string vDataMulta = boleto.DataVencimento.AddDays(1).ToString("ddMMyy") ;
                if (!boleto.DataMulta.Equals(DateTime.MinValue))
                    vDataMulta = boleto.DataMulta.ToString("ddMMyy");
                decimal vlMulta = ((boleto.ValorBoleto * boleto.PercMulta)/100);//.ToString().Replace(",", "").Replace(".", "");
                string valMulta = string.Format("{0:0.00}", vlMulta).Replace(",", "").Replace(".", "");
                #endregion
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediNumericoSemSeparador_, 0352, 006, 0, vDataMulta, '0'));                                //352-357
                reg.CamposEDI.Add(new TCampoRegistroEDI(TTiposDadoEDI.ediNumericoSemSeparador_, 0358, 010, 2, valMulta, '0'));                                  //358-367
