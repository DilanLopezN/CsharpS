    ...
    internal class Banco_Itau : AbstractBanco, IBanco
    {
        ...
        public string GerarDetalheRemessaCNAB400(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                // LINHAS COMENTADAS PARA ATENDER O CHAMADO 261179, NOVA IMPLEMENTAÇÃO SEGUE NA REGION "Outros Descontos":
                ...
                #region Outros Descontos
                // Posição 352 a 353 : Brancos 
                _detalhe += "  ";

                // Posição 354 a 359 : Data do 2º desconto (DDMMAA)
                if (boleto.DataOutrosDescontos == DateTime.MinValue || boleto.OutrosDescontos == 0)
                ...
                if (boleto.DataOutrosDescontos2 == DateTime.MinValue || boleto.OutrosDescontos2 == 0)
                    _detalhe += "000000";
                else
                    _detalhe += boleto.DataOutrosDescontos2.ToString("ddMMyy");

                // Posição 379 a 391 : Valor do 3º desconto Março 2015 Cobrança CNAB 400 23
                ...
                if (boleto.Instrucoes.Count > 0)
                {
                    for (int i = 0; i < boleto.Instrucoes.Count; i++)
                    {
                        if (boleto.Instrucoes[i].Codigo == (int)EnumInstrucoes_Itau.Protestar || 
                            ...
                            boleto.Instrucoes[i].Codigo == (int) EnumInstrucoes_Itau.MsgBoleto40Posicoes)
                            {
