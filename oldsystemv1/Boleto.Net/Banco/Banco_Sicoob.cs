...
namespace BoletoNet
{
    ...
    internal class Banco_Sicoob : AbstractBanco, IBanco
    {
        ...
        public override void ValidaBoleto(Boleto boleto)
        {
            ...
            //Atribui o nome do banco ao local de pagamento
            //Alterado conforme chamado 249378
            ...
            //Alterado conforme chamado 278262
        ...
        private string GerarHeaderRemessaCNAB240(int numeroConvenio, Cedente cedente, int numeroArquivoRemessa)
        {
            ...
            try
            {
                ...
                header += "0";         //Posição 008 a 008      Tipo de Registro: "0"
                header += new string(' ', 9); //); //Posição 009 a 017      Uso Exclusivo FEBRABAN / CNAB: Brancos
                header += cedente.CPFCNPJ.Length == 11 ? "1" : "2"; //Posição 018 a 018      1=CPF    2=CGC/CNPJ
                ...
                header += Utils.FormatCode(cedente.ContaBancaria.DigitoAgencia, " ", 1); //Posição 058 a 058      Dígito Verificador do Prefixo: vide planilha "Capa" deste arquivo
                header += Utils.FormatCode(cedente.ContaBancaria.Conta, "0", 12, true);  //Posição 059 a 070      Conta Corrente: vide planilha "Capa" deste arquivo
                header += Utils.FormatCode(cedente.ContaBancaria.DigitoConta, " ", 1);   //Posição 071 a 071      Dígito Verificador da Conta: vide planilha "Capa" deste arquivo
                header += new string('0', 1); //Posição 072 a 072      Dígito Verificador da Ag/Conta: Brancos
                header += Utils.FitStringLength(cedente.Nome, 30, 30, ' ', 0, true, true, false).ToUpper(); //Posição 073 a 102      Nome da Empresa
                header += Utils.FormatCode("SICOOB", " ", 30); //Posição 103 a 132      Nome do Banco: SICOOB
                ...
                header += Utils.FormatCode("", " ", 20);       //Posição 172 a 191      Para Uso Reservado do Banco: Brancos
                header += Utils.FormatCode("", " ", 20);       //Posição 192 a 211      Para Uso Reservado da Empresa: Brancos
                header += Utils.FormatCode("", " ", 29);       //Posição 212 a 240      Uso Exclusivo FEBRABAN / CNAB: Brancos
        ...
        private string GerarHeaderLoteRemessaCNAB240(Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                string header = "756";          //Posição 001 a 003      Código do Banco na Compensação: "756"
                header += "0001";               //Posição 004 a 007      Número Sequencial: Preencher com '0001' para o primeiro lote do arquivo. Para os demais: número do lote anterior acrescido de 1. O número não poderá ser repetido dentro do arquivo."
                header += "1";                  //Posição 008 a 008      Tipo de Registro: "1"
                header += "R";                  //Posição 009 a 009      Tipo de Operação: "R"
                header += "01";                 //Posição 010 a 011      Tipo de Serviço: "01"
                header += new string(' ', 2);   //Posição 012 a 013      Uso Exclusivo FEBRABAN/CNAB: Brancos
                header += "040";                //Posição 014 a 016      Nº da Versão do Layout do Lote: "040"
                header += new string(' ', 1);   //Posição 017 a 017      Uso Exclusivo FEBRABAN/CNAB: Brancos
                header += (cedente.CPFCNPJ.Length == 11 ? "1" : "2");        //Posição 018 a 018      1=CPF    2=CGC/CNPJ
                header += Utils.FormatCode(cedente.CPFCNPJ, "0", 15, true);  //Posição 019 a 033      Nº de Inscrição da Empresa
                header += Utils.FormatCode("", " ", 20, true);               //Posição 034 a 053      Código do Convênio no Banco: Brancos
                ...
                header += Utils.FormatCode(cedente.ContaBancaria.DigitoAgencia, " ", 1, true); // Posição 059 a 059     Dígito Verificador do Prefixo: vide planilha "Capa" deste arquivo
                header += Utils.FormatCode(cedente.ContaBancaria.Conta, "0", 12, true);        // Posição 060 a 071     Conta Corrente: vide planilha "Capa" deste arquivo
                header += Utils.FormatCode(cedente.ContaBancaria.DigitoConta, " ", 1, true);   // Posição 072 a 072     Dígito Verificador da Conta: vide planilha "Capa" deste arquivo    
                header += new string(' ', 1);                                                  // Posição 073 a 073     Dígito Verificador da Ag/Conta: Brancos
                header += Utils.FitStringLength(cedente.Nome, 30, 30, ' ', 0, true, true, false).ToUpper();  // Posição 074 a 103      Nome da Empresa
                header += Utils.FormatCode("", " ", 40);                                    // Posição 104 a 143      Informação 1 - Mensagem 1
                header += Utils.FormatCode("", " ", 40);                                    // Posição 144 a 183      Informação 2 - Mensagem 2
                header += Utils.FormatCode(numeroArquivoRemessa.ToString(), "0", 8, true);  // Posição 184 a 191      Número Remessa/Retorno
        ...
        private string GerarDetalheRemessaCNAB400(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            ...
            try
            {
                ...
                #region Instruções
                string vInstrucao1 = "07"; //2ª instrução (2, N) Caso Queira colocar um cod de uma instrução. ver no Manual caso nao coloca 00
                ...
                foreach (IInstrucao instrucao in boleto.Instrucoes)
                {
                    switch ((EnumInstrucoes_Sicoob)instrucao.Codigo)
                    {
                        case EnumInstrucoes_Sicoob.Protestar:
                            vInstrucao1 = Utils.FitStringLength(instrucao.Codigo.ToString(), 2, 2, '0', 0, true, true, true);
                            vInstrucao2 = Utils.FitStringLength(instrucao.QuantidadeDias.ToString(), 2, 2, '0', 0, true, true, true);
                            break;
                    }
                }
                #endregion
                _detalhe.Append(vInstrucao1); //Posição 157 a 158 - Instruções
                ...
                _detalhe.Append(vInstrucao2); //Posição 392 a 393 - DIAS PARA PROTESTO
        ...
        public override string GerarDetalheSegmentoPRemessa(Boleto boleto, int numeroRegistro, string numeroConvenio)
        {
            try
            {
                string detalhe = Utils.FormatCode(Codigo.ToString(), 3);               //Posição 001 a 003      Código do Banco na Compensação: "756"
                detalhe += "0001";  //Posição 004 a 007      Lote de Serviço: Preencher com '0001' para o primeiro lote do arquivo. Para os demais: número do lote anterior acrescido de 1. O número não poderá ser repetido dentro do arquivo."
                detalhe += "3";                                                        //Posição 008 a 008      Tipo de Registro: "3"
                detalhe += Utils.FormatCode(numeroRegistro.ToString(), "0", 5, true);  //Posição 009 a 013      "Nº Sequencial do Registro no Lote: Número adotado e controlado pelo responsável pela geração magnética dos dados contidos no arquivo, para identificar a seqüência de registros encaminhados no lote. Deve ser inicializado sempre em '1', em cada novo lote."
                detalhe += "P";                                                        //Posição 014 a 014      Cód. Segmento do Registro Detalhe: "P"
                detalhe += " ";                                                        //Posição 015 a 015      Uso Exclusivo FEBRABAN/CNAB: Brancos
                detalhe += Utils.FormatCode(boleto.Remessa.CodigoOcorrencia, 2);       //Posição 016 a 017      Código de Movimento Remessa: '01'  =  Entrada de Títulos
                ...
                detalhe += Utils.FormatCode(boleto.Cedente.ContaBancaria.DigitoAgencia, " ", 1, false);  //Posição 023 a 023     Dígito Verificador do Prefixo: vide planilha "Capa" deste arquivo
                ...
                detalhe += Utils.FormatCode(boleto.Cedente.ContaBancaria.DigitoConta, 1); //Posição 036 a 036     Dígito Verificador da Conta: vide planilha "Capa" deste arquivo
                detalhe += " ";                                                           //Posição 037 s 037     Dígito Verificador da Ag/Conta: Brancos
                detalhe += Utils.FitStringLength(boleto.NossoNumero, 15, 15, ' ', 0, true, true, false);                      //Posição 038 a 057     Nosso Número
                detalhe += new String(' ', 5);
                detalhe += (Convert.ToInt16(boleto.Carteira) == 1 ? "1" : "2");           //Posição 058 a 058     Código da Carteira: vide planilha "Capa" deste arquivo
                detalhe += "0";  //Posição 059 a 059     Forma de Cadastr. do Título no Banco: "0"
                detalhe += " ";  //Posição 060 a 060     Tipo de Documento: Brancos
                detalhe += "2";  //Posição 061 a 061     "Identificação da Emissão do Boleto: (vide planilha ""Capa"" deste arquivo) '1'  =  Sicoob Emite  '2'  =  Beneficiário Emite" TODO:Deivid
                detalhe += "2";  //Posição 062 a 062     "Identificação da Distribuição do Boleto: (vide planilha ""Capa"" deste arquivo) '1'  =  Sicoob Distribui  '2'  =  Beneficiário Distribui" TODO:Deivid
                detalhe += Utils.FormatCode(boleto.NumeroDocumento, 15); //Posição 063 a 077     Número do Documento de Cobrança. TODO:Deivid
                detalhe += Utils.FormatCode(boleto.DataVencimento.ToString("ddMMyyyy"), 8); //Posição 078 a 085     Data de Vencimento do Título
                string valorBoleto = boleto.ValorBoleto.ToString("f").Replace(",", "").Replace(".", "");
                valorBoleto = Utils.FormatCode(valorBoleto, 15);
                detalhe += valorBoleto; //Posição 086 a 100     Valor Nominal do Título
                detalhe += Utils.FormatCode(boleto.ContaBancaria.Agencia, 5); //Posição 101 a 105     Agência Encarregada da Cobrança: "00000"
                detalhe += new string(' ', 1);  //Posição 106 a 106    Dígito Verificador da Agência: Brancos
                detalhe += Utils.FormatCode(boleto.EspecieDocumento.Codigo, 2);  //Posição 107 a 108     Espécie de Título
                detalhe += Utils.FormatCode(boleto.Aceite, 1);  //Posição 109 a 109    Identific. de Título Aceito/Não Aceito: 'A'  =  Aceite  'N'  =  Não Aceite"  TODO:Deivid
                detalhe += Utils.FormatCode(boleto.DataProcessamento.ToString("ddMMyyyy"), 8);   //Posição 110 a 117     Data da Emissão do Título
                
                // Código do Juros de Mora
                detalhe += "2"; //Posição 118 a 118     Código do Juros de Mora: '0'  =  Isento  '1'  =  Valor por Dia  '2'  =  Taxa Mensal
                // Data do Juros Mora
                string dataJurosMora;
                if (boleto.PercJurosMora == 0)
                {
                    dataJurosMora = "00000000";
                }
                else if (boleto.DataJurosMora > boleto.DataVencimento) 
                {
                    dataJurosMora = Utils.FormatCode(boleto.DataJurosMora.ToString("ddMMyyyy"), "0", 8, true);
                }
                else
                {
                    dataJurosMora = Utils.FormatCode(boleto.DataVencimento.AddDays(1).ToString("ddMMyyyy"), "0", 8, true);
                }
                detalhe += dataJurosMora; //Posição 119 a 126     Data do Juros de Mora: preencher com a Data de Vencimento do Título

                // Juros de Mora por Dia/Taxa ao Mês:  Valor = R$ ao dia / Taxa = % ao mês -  Taxa Mensal, conforme Posição 118 a 118
                string percJurosMora = Convert.ToInt32(boleto.PercJurosMora * 100).ToString(); // Taxa Mensal, conforme Posição 118 a 118
                percJurosMora = Utils.FormatCode(percJurosMora, 15);
                detalhe += percJurosMora; //Posição 127 a 141     Juros de Mora por Dia/Taxa ao Mês:  Valor = R$ ao dia / Taxa = % ao mês

                // 1º Desconto
                if (boleto.ValorDesconto > 0)
                {
                    detalhe += "1"; //Posição 142 a 142     Código do Desconto 1: '0'  =  Não Conceder desconto / '1'  =  Valor Fixo Até a Data Informada / '2'  =  Percentual Até a Data Informada
                }
                else
                {
                    detalhe += "0"; //Posição 142 a 142     Código do Desconto 1: '0'  =  Não Conceder desconto / '1'  =  Valor Fixo Até a Data Informada / '2'  =  Percentual Até a Data Informada
                }
                if (boleto.DataDesconto == DateTime.MinValue || boleto.ValorDesconto == 0) //Posição 143 a 150     Data do Desconto 1
                {
                    detalhe += "00000000";
                }
                else
                ...
                valorBoleto = Utils.FormatCode(valorBoleto, "0", 15, true);
                detalhe += valorBoleto; //Posição 151 a 165     Valor/Percentual a ser Concedido - Desconto 1
                ...
                //detalhe += Utils.FormatCode(boleto.NumeroDocumento, 25); //Posição 196 a 220     Identificação do Título na Empresa: Campo destinado para uso do Beneficiário para identificação do Título.
                detalhe += Utils.FormatCode(boleto.NumeroDocumento.ToString(), " ", 25, false); //Posição 196 a 220     Identificação do Título na Empresa: Campo destinado para uso do Beneficiário para identificação do Título.
                //detalhe += "3"; //Posição 221 a 221     Código p/ Protesto: '1' = Protestar dias corridos; '3' = Não Protestar; e '9' = Cancelar Instrução de Protesto
                ...
                #endregion         
                if (vInstrucao1 == "00") //Posição 221 a 221     Código p/ Protesto: '1' = Protestar dias corridos; '3' = Não Protestar; e '9' = Cancelar Instrução de Protesto
                {
                    detalhe += "3"; //Posição 221 a 221     Código p/ Protesto: "3" = Não Protestar
                }
                else
                {
                    detalhe += "1"; //Posição 221 a 221     Código para Protesto: "1" = Protestar dias corridos
                }
                detalhe += Utils.FormatCode(vInstrucao1, 2);  //Posição 222 a 223     Número de Dias Corridos para Protesto
                detalhe += Utils.FormatCode("0", 1);          //Posição 224 a 224     Código para Baixa/Devolução: "0"
                detalhe += new string(' ', 3); //Posição 225 a 227     Número de Dias para Baixa/Devolução: Brancos
                detalhe += Utils.FormatCode(boleto.Moeda.ToString(), "0", 2, true); //Posição 228 a 229     Código da Moeda: '02'  =  Dólar Americano Comercial (Venda) / '09'  = Real
                detalhe += Utils.FormatCode("", "0", 10, true); //Posição 230 a 239     Nº do Contrato da Operação de Créd.: "0000000000"
                detalhe += " "; //Posição 240 a 240     Uso Exclusivo FEBRABAN/CNAB: Brancos
        ...
        public override string GerarDetalheSegmentoQRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                string detalhe = Utils.FormatCode(Codigo.ToString(), "0", 3, true); //Posição 001 a 003     Código do Banco na Compensação: "756"
                detalhe += "0001"; //Posição 004 a 007     Número Sequencial - Lote de Serviço: Número seqüencial
                detalhe += "3"; //Posição 008 a 008  Tipo de Registro: "3"
                detalhe += Utils.FormatCode(numeroRegistro.ToString(), "0", 5, true); //Posição 009 a 013     Número Sequencial - Nº Sequencial do Registro no Lote
                detalhe += "Q"; //Posição 014 a 014     Cód. Segmento do Registro Detalhe: "Q"
                detalhe += " ";  //Posição 015 a 015     Uso Exclusivo FEBRABAN/CNAB: Brancos
                detalhe += "01"; //Posição 016 a 017     Código de Movimento Remessa: '01'  =  Entrada de Títulos
                detalhe += (boleto.Sacado.CPFCNPJ.Length == 11 ? "1" : "2");  //Posição 018 a 018     Tipo de Inscrição Pagador: '1'  =  CPF / '2'  =  CGC / CNPJ - Dados do Pagador
                detalhe += Utils.FormatCode(boleto.Sacado.CPFCNPJ, "0", 15, true); //Posição 019 a 033     Número de Inscrição - Dados do Pagador
                detalhe += Utils.FitStringLength(boleto.Sacado.Nome, 40, 40, ' ', 0, true, true, false);  //Posição 034 a 073     Nome - Dados do Pagador
                detalhe += Utils.FitStringLength(boleto.Sacado.Endereco.End, 40, 40, ' ', 0, true, true, false);  //Posição 074 a 113     Endereço - Dados do Pagador
                detalhe += Utils.FitStringLength(boleto.Sacado.Endereco.Bairro, 15, 15, ' ', 0, true, true, false); //Posição 114 a 128     Bairro  - Dados do Pagador
                detalhe += Utils.FormatCode(boleto.Sacado.Endereco.CEP, "0", 8, true); //Posição 129 a 136     CEP (5, N) + Sufixo do CEP (3, N) Total (8, N) - Dados do Pagador
                detalhe += Utils.FitStringLength(boleto.Sacado.Endereco.Cidade, 15, 15, ' ', 0, true, true, false); //Posição 137 a 151     Cidade  - Dados do Pagador
                detalhe += Utils.FormatCode(boleto.Sacado.Endereco.UF, " ", 2, false); //Posição 152 a 153     UF  - Unidade da Federação - Dados do Pagador
                detalhe += (boleto.Cedente.CPFCNPJ.Length == 11 ? "1" : "2"); //Posição 154 a 154     Tipo de Inscrição Sacador Avalista: '1'  =  CPF / '2'  =  CGC / CNPJ
                detalhe += Utils.FormatCode(boleto.Cedente.CPFCNPJ, "0", 15, true); //Posição 155 a 169     Número de Inscrição / Sacador avalista
                detalhe += Utils.FitStringLength(boleto.Cedente.Nome, 40, 40, ' ', 0, true, true, false).ToUpper(); //Posição 170 a 209     Nome do Sacador/Avalista
                detalhe += "000"; //Posição 210 a 212     Cód. Bco. Corresp. na Compensação
                detalhe += Utils.FormatCode("", " ", 20, false); //Posição 213 a 232     Nosso N° no Banco Correspondente "1323739"
                detalhe += Utils.FormatCode("", " ", 8, false);  //Posição 233 a 240     Uso Exclusivo FEBRABAN/CNAB
        ...
        public override string GerarDetalheSegmentoRRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                detalhe += "3"; //Posição 008 a 008     Tipo de Registro: "3"
                detalhe += Utils.FormatCode(numeroRegistro.ToString(), "0", 5, true); //Posição 009 a 013     Número Sequencial - Nº Sequencial do Registro no Lote
                detalhe += "R"; //Posição 014 a 014     Cód. Segmento do Registro Detalhe: "R"
                detalhe += " "; //Posição 015 a 015     Uso Exclusivo FEBRABAN/CNAB: Brancos
                detalhe += "01"; //Posição 016 a 017     Código de Movimento Remessa: '01'  =  Entrada de Títulos
                // 2º Desconto
                if (boleto.DataOutrosDescontos == DateTime.MinValue || boleto.OutrosDescontos == 0) //Posição 018 a 018     Código do Desconto 2: '0'  =  Não Conceder desconto / '1'  =  Valor Fixo Até a Data Informada / '2'  =  Percentual Até a Data Informada
                {
                    detalhe += "0";
                }
                else
                ...
                if (boleto.DataOutrosDescontos == DateTime.MinValue || boleto.OutrosDescontos == 0) //Posição 019 a 026     Data do Desconto 2
                {
                    detalhe += "00000000";
                }
                else
                {
                    detalhe += Utils.FormatCode(boleto.DataOutrosDescontos.ToString("ddMMyyyy"), "0", 8, true);
                }
                string valorDesconto2 = boleto.OutrosDescontos.ToString("f").Replace(",", "").Replace(".", "");
                valorDesconto2 = Utils.FormatCode(valorDesconto2, "0", 15, true);
                detalhe += valorDesconto2; //Posição 027 a 041     Valor/Percentual a ser Concedido - Desconto 2

                // 3º Desconto
                if (boleto.DataOutrosDescontos2 == DateTime.MinValue || boleto.OutrosDescontos2 == 0) //Posição 042 a 042     Código do Desconto 3: '0'  =  Não Conceder desconto / '1'  =  Valor Fixo Até a Data Informada / '2'  =  Percentual Até a Data Informada
                {
                    detalhe += "0";
                }
                else
                ...
                if (boleto.DataOutrosDescontos2 == DateTime.MinValue || boleto.OutrosDescontos2 == 0) //Posição 043 a 050     Data do Desconto 3: "00000000"
                {
                    detalhe += "00000000";
                }
                else
                {
                    detalhe += Utils.FormatCode(boleto.DataOutrosDescontos2.ToString("ddMMyyyy"), "0", 8, true);
                }
                string valorDesconto3 = boleto.OutrosDescontos2.ToString("f").Replace(",", "").Replace(".", "");
                valorDesconto3 = Utils.FormatCode(valorDesconto3, "0", 15, true);
                detalhe += valorDesconto3; //Posição 051 a 065     Valor/Percentual a ser Concedido: "000000000000000" - Desconto 3

                if (boleto.PercMulta > 0) //Posição 066 a 066     Código da Multa: '0'  =  Isento / '1'  =  Valor Fixo / '2'  =  Percentual
                {
                ...
                {
                    // Código da Multa: '0'  =  Isento - sem multa
                    detalhe += "0";
                }

                if (boleto.DataMulta.Equals(boleto.DataVencimento))  //Posição 067 a 074     Data da Multa
                {
                    detalhe += Utils.FormatCode(boleto.DataMulta.AddDays(1).ToString("ddMMyyyy"), "0", 8, true);

                }else
                {
                    detalhe += Utils.FormatCode(boleto.DataMulta.ToString("ddMMyyyy"), "0", 8, true); 
                }

                if (boleto.PercMulta > 0) //Posição 075 a 089     Valor/Percentual a Ser Aplicado
                {
                ...
                detalhe += Utils.FormatCode("", " ", 10, false); //Posição 090 a 099     Informação ao Pagador: Brancos
                detalhe += Utils.FormatCode("", " ", 40, false); //Posição 100 a 139     Informação 3 - Mensagem 3
                detalhe += Utils.FormatCode("", " ", 40, false); //Posição 140 a 179     Informação 4 - Mensagem 4
                detalhe += Utils.FormatCode("", " ", 20, false); //Posição 180 a 199     Uso Exclusivo FEBRABAN/CNAB: Brancos
                ...
                detalhe += " "; //Posição 216 a 216     Dígito Verificador da Agência: Brancos
                detalhe += Utils.FormatCode("", "0", 12, true);  //Posição 217 a 228     Conta Corrente para Débito: "000000000000"
                detalhe += " "; //Posição 229 a 229     Dígito Verificador da Conta: Brancos
                detalhe += " "; //Posição 230 a 230     Dígito Verificador Ag/Conta: Brancos
                detalhe += "0"; //Posição 231 a 231     Aviso para Débito Automático: "0"
                detalhe += Utils.FormatCode("", " ", 9, false); //Posição 232 a 240     Uso Exclusivo FEBRABAN/CNAB: Brancos
                detalhe = Utils.SubstituiCaracteresEspeciais(detalhe);
                return detalhe;
            }
            catch (Exception e)
        ...
        public override DetalheSegmentoURetornoCNAB240 LerDetalheSegmentoURetornoCNAB240(string registro)
        {
            try
            {
                ...
                detalhe.DataCredito = Utils.ToDateTime(Utils.ToInt32(registro.Substring(145, 8)).ToString("##-##-####"));
                ...
                detalhe.DataOcorrencia = Utils.ToDateTime(Utils.ToInt32(registro.Substring(137, 8)).ToString("##-##-####"));
