    ...
    internal class Banco_Brasil : AbstractBanco, IBanco
    {
        ...
        public override string GerarDetalheSegmentoPRemessa(Boleto boleto, int numeroRegistro, string numeroConvenio)
        {
            try
            {
                ...
                _segmentoP += Utils.FitStringLength(_nossoNumero, 20, 20, ' ', 0, true, true, false);//pos(57)
                ...
                {
                    _segmentoP += "1";//Posição 142 a 142     
                }
                else
                {
                    _segmentoP += "0";//Posição 142 a 142  
                }
                ...
                {
                    _segmentoP += Utils.FormatCode(boleto.DataDesconto.ToString("ddMMyyyy"), 8); //Posição 143 a 150     Data do Desconto 1
                }

                var valorBoleto = boleto.ValorDesconto.ToString("f").Replace(",", "").Replace(".", "");
                valorBoleto = Utils.FormatCode(valorBoleto, "0", 15, true);
                _segmentoP += valorBoleto; //Posição 151 a 165     Valor/Percentual a ser Concedido - Desconto 1
        ...
        public override string GerarDetalheSegmentoRRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                _segmentoR = "00100013"; //(Qtd: 8 | 1 a 8): De 1 a 3 -> Código do Banco na Compensação | De 4 a 7 -> Lote de Serviço | De 8 a 8  Tipo de Registro 
                _segmentoR += Utils.FitStringLength(numeroRegistro.ToString(), 5, 5, '0', 0, true, true, true);//(Qtd: 5 | 9 a 13): Nº Sequencial do Registro no Lote 
                _segmentoR += "R 01";//(Qtd: 4 | 14 a 17): De 14 a 14 -> Cód. Segmento do Registro Detalhe  | De 15 a 15 -> Uso Exclusivo FEBRABAN/CNAB  | De 16 a 17  Código de Movimento Remessa  
                // 2º Desconto (Qtd: 24 | 18 a 41)
                if (boleto.DataOutrosDescontos == DateTime.MinValue || boleto.OutrosDescontos == 0) //Posição 018 a 018     
                {
                    _segmentoR += "0";
                }
                else
                ...
                {
                    _segmentoR += Utils.FormatCode(boleto.DataOutrosDescontos.ToString("ddMMyyyy"), "0", 8, true);
                }
                ...
                _segmentoR += valorDesconto2; //Posição 027 a 041     Valor/Percentual a ser Concedido - Desconto 2

                // Desconto 3 (Qtd: 24 | 42 a 65)
                if (boleto.DataOutrosDescontos2 == DateTime.MinValue || boleto.OutrosDescontos2 == 0) //Posição 042 a 042     Código do Desconto 3
                {
                    _segmentoR += "0";
                }
                else
                ...
                {
                    _segmentoR += Utils.FormatCode(boleto.DataOutrosDescontos2.ToString("ddMMyyyy"), "0", 8, true);
                }
                ...
                _segmentoR += valorDesconto3; //Posição 051 a 065     Valor/Percentual a ser Concedido: "000000000000000" - Desconto 3

                // Código da multa - percentual (66 a 66)
        ...
        public override string GerarTrailerLoteRemessa(int numeroRegistro)
        {
            try
            {
                ...
                int vQtdeRegLote = numeroRegistro; //(numeroRegistro + 1); // (numeroRegistro + 2);
        ...
        public string GerarHeaderRemessaCNAB240(Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                // para a formatação dos campos.
                // Como não sei onde pegar esse nº, deixei como padrão.
                _header += "083"; //anteriormente estava 050 (foi alterado para 083 conforme chamado 274001)
                _header += "00000";
                _header += _brancos20;
                _header += _brancos20;
                _header += _brancos10;
