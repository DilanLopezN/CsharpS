...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioRetornoTitulosCNAB.aspx");
            ...
            private void bindRelatorio(string url){
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if (parametrosHash[0].Equals("cd_retorno_cnab"))
                            cd_retorno_cnab = int.Parse(parametrosHash[1]);
                        else
                        {
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        List<FundacaoFisk.SGF.Web.Services.CNAB.Model.RptTitulosCnab> itens = apresentadorRelatorio.getSourceTituloRetornoCNAB(cd_retorno_cnab).ToList();
                        ...
                        if (sourceHash != null && sourceHash.Values.Count > 0)
                            relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_TITULOS_RETORNO_CNAB, sourceHash, parametrosRelatorio);
                        else
                        {
                            //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
                            url = FundacaoFisk.SGF.ApresentacaoRelatorio.comum.UrlErro.ObterUrlErroRelatorio(url);
