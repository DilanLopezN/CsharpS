...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioEspelhoMovimento.aspx");
            ...
            private void bindRelatorio(string url){
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if (parametrosHash[0].Equals("@cd_movimento"))
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        FundacaoFisk.SGF.Web.Services.Financeiro.Model.Espelho espelho = apresentadorRelatorio.getSourceEspelhoMovimento(cd_movimento, cdEscola);
                        List<FundacaoFisk.SGF.GenericModel.ItemMovimento> itens = apresentadorRelatorio.getSourceItensMovimento(cd_movimento, cdEscola).ToList();
                        List<FundacaoFisk.SGF.GenericModel.Titulo> titulos = apresentadorRelatorio.getSourceTitulosMovimento(cd_movimento, cdEscola).ToList();
                        
                        List<Componentes.GenericModel.TO> sourceTO = new List<Componentes.GenericModel.TO>();
                        sourceTO.Add(espelho);
                        Hashtable sourceHash = new Hashtable();
                        sourceHash["DataSetReport"] = sourceTO;
                        sourceTO = new List<Componentes.GenericModel.TO>(itens);
                        sourceHash["DataSetSubReportItens"] = sourceTO;
                        sourceTO = new List<Componentes.GenericModel.TO>(titulos);
                        sourceHash["DataSetSubReportTitulos"] = sourceTO;

                        if (sourceHash != null && sourceHash.Values.Count > 0)
                            relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_ESPELHO_MOVIMENTO, sourceHash, parametrosRelatorio);
                        else
                        {
                            //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
                            url = FundacaoFisk.SGF.ApresentacaoRelatorio.comum.UrlErro.ObterUrlErroRelatorio(url);
