...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioAlunosSemTituloGerado.aspx");
            ...
            private void bindRelatorio(string url){
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if (parametrosHash[0].Equals("@vl_mes"))
                            vl_mes = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@no_mes"))
                            no_mes = parametrosHash[1] + "";
                        ...
                        if (parametrosHash[0].Equals("@situacoes"))
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        IEnumerable<AlunosSemTituloGeradoUI> alunosSemTituloGerado = apresentadorRelatorio.GetSourceAlunosSemTituloGeradoReportView(vl_mes, ano, cd_turma, situacoes, cdEscola);

                        dynamic obj = new Object();

                        obj = alunosSemTituloGerado.ToList<Componentes.GenericModel.TO>();

                        parametrosRelatorio.Add("PMes", no_mes);
                        parametrosRelatorio.Add("PAno", ano);
                        parametrosRelatorio["PTitulo"] = "Relatório de Alunos que não possuem títulos gerados";
                       // parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_RELATORIO, "Relatório de Alunos que não possuem títulos gerados");

                        List<Componentes.GenericModel.TO> sourceTO = obj;

                        if (sourceTO != null && sourceTO.ToList<Componentes.GenericModel.TO>().Count > 0)
                            relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_ALUNOS_SEM_TITULO_GERADO, sourceTO, parametrosRelatorio);
                        else
                        {
                            //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
                            url = FundacaoFisk.SGF.ApresentacaoRelatorio.comum.UrlErro.ObterUrlErroRelatorio(url);
