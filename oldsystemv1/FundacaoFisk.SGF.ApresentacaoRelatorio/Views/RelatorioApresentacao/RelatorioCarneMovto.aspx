...
<body>
    <div>
     <script runat="server">
            ...
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioCarneMovto.aspx");
            ...
            private void bindRelatorio(string url) { 
                ...
                if (parms != null)
                {
                    ...
                    for (int i = 0; i < parametrosGet.Length; i++)
                    {
                        ...
                        if (parametrosHash[0].Equals("@cdMovimento"))
                            cdMovimento = int.Parse(parametrosHash[1]);
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        IEnumerable<FundacaoFisk.SGF.Web.Services.Financeiro.Model.CarneUI> carnes = apresentadorRelatorio.getCarnePorMovimentos(cdMovimento, cdEscola, contaSegura).ToList();


                        var nomeRelatorio = FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_CARNE_MOVTO;
                     ...
                     //   string tituloRel = "Carnê";
                      ...
                      //  parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_RELATORIO, tituloRel);
                        ...
                        else
                        {
                            //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
                            url = FundacaoFisk.SGF.ApresentacaoRelatorio.comum.UrlErro.ObterUrlErroRelatorio(url);
                            Response.Redirect(url + "?msgErro=" + HttpUtility.UrlEncode(FundacaoFisk.SGF.Utils.Messages.Messages.msgErroCarne) + "&stackTrace=");
                        }
                    }
                    else
                    {
