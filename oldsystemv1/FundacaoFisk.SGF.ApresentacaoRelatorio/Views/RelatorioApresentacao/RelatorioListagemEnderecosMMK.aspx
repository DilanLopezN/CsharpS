...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioCopiaEspelhoMovimento.aspx");
            ...
            private void bindRelatorio(string url){
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if (parametrosHash[0].Equals("@cd_cnab"))
                            cd_cnab = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@cd_mala_direta"))
                            cd_mala_direta = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@no_pessoa"))
                            no_pessoa = parametrosHash[1];
                        if (parametrosHash[0].Equals("@status"))
                            status = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@email"))
                            email = parametrosHash[1];
                        if (parametrosHash[0].Equals("@id_tipo_cadastro"))
                            id_tipo_cadastro = byte.Parse(parametrosHash[1]);
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        List<FundacaoFisk.SGF.Web.Service.EmailMarketing.Model.RptListagemEndereco> listagem = apresentadorRelatorio.getRptListagemEnderecos(cd_mala_direta, cd_empresa, no_pessoa, status,
                            email,id_tipo_cadastro).ToList();
                        parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_RELATORIO, "Listagem de Endereços");
                        List<Componentes.GenericModel.TO> sourceTO = new List<Componentes.GenericModel.TO>();


                        sourceTO = new List<Componentes.GenericModel.TO>(listagem);
                        Hashtable sourceHash = new Hashtable();
                        sourceHash["DataSetReport"] = sourceTO;
                        

                        if (sourceHash != null && sourceHash.Values.Count > 0)
                            relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_LISTAGEM_ENDERECOS_MMK, sourceHash, parametrosRelatorio);
                        else
                        {
                            //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
                            url = FundacaoFisk.SGF.ApresentacaoRelatorio.comum.UrlErro.ObterUrlErroRelatorio(url);
