...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioControleFaltas.aspx");
            ...
            private void bindRelatorio(string url){
                ...
                /*
                DateTime? dt_inicial = DateTime.MinValue;
                DateTime? dt_final = DateTime.MaxValue;
                if (dataIni.HasValue)
                ...
                */
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if (parametrosHash[0].Equals("@cd_tipo"))
                            cd_tipo = int.Parse(parametrosHash[1]);
                        ...
                        if (parametrosHash[0].Equals("@cd_nivel"))
                            cd_nivel = int.Parse(parametrosHash[1]);
                        ...
                        if (parametrosHash[0].Equals("@cd_sit_turma"))
                            cd_sit_turma = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@cd_sit_aluno"))
                            cd_sit_aluno = parametrosHash[1];
                        if (parametrosHash[0].Equals("@quebrarpagina"))
                            quebrarpagina = bool.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@dt_inicial"))
                            dt_inicial = !String.IsNullOrEmpty(parametrosHash[1]) ? parametrosHash[1] : "01/01/1900";
                        if (parametrosHash[0].Equals("@dt_final"))
                            dt_final = !String.IsNullOrEmpty(parametrosHash[1]) ? parametrosHash[1] : "06/06/2079";
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        List<FundacaoFisk.SGF.GenericModel.sp_RptControleFaltas_Result> controleFaltasResult = apresentadorRelatorio.GetSourceControleFaltasResults(cd_tipo, cd_escola, cd_curso, cd_nivel, cd_produto, cd_professor, cd_turma, cd_sit_turma, cd_sit_aluno, dt_inicial, dt_final, quebrarpagina);
                        List<Componentes.GenericModel.TO> sourceTO = null;
                        if (controleFaltasResult.Count() > 0)
                        {

                            sourceTO = controleFaltasResult.ToList<Componentes.GenericModel.TO>();
                        }
                        parametrosRelatorio["PEscola"] = parametrosRelatorio["PEmpresa"];
                        parametrosRelatorio.Remove("PEmpresa");
                        parametrosRelatorio["PUsuario"] = parametrosRelatorio["PNomeUsuario"];
                        parametrosRelatorio.Remove("PNomeUsuario");

                        parametrosRelatorio.Remove("PTipoRelatorio");

                        parametrosRelatorio.Add("Pdt_inicial", dt_inicial);
                        parametrosRelatorio.Add("Pdt_final", dt_final);
                        parametrosRelatorio.Add("PPagina", quebrarpagina);
                        ...
                        {
                            sourceHash["DataRelatorio"] = controleFaltasResult;
                            relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_CONTROLE_FALTAS, sourceHash, parametrosRelatorio);
                        }
                        else
                        {
                            //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
