...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioAulaReposicaoView.aspx");
            ...
            private void bindRelatorio(string url){
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if(parametrosHash[0].Equals("@cdAulaReposicao"))
                            cdAulaReposicao = int.Parse(parametrosHash[1]);
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        FundacaoFisk.SGF.GenericModel.AulaReposicao aulaReposicao = apresentadorRelatorio.GetSourceAulaReposicaoViewReportView(cdAulaReposicao);

                        // Inclui os parâmetros especializados do relatório de atividade extra:
                        parametrosRelatorio.Add("PData", String.Format("{0:dd/MM/yyyy}", aulaReposicao.dt_aula_reposicao));
                        parametrosRelatorio.Add("PResponsavel", aulaReposicao.Professor.FuncionarioPessoaFisica != null ? aulaReposicao.Professor.FuncionarioPessoaFisica.no_pessoa : "");
                        parametrosRelatorio.Add("PHorario", aulaReposicao.tempo);
                        parametrosRelatorio.Add("PObservacao", aulaReposicao.tx_observacao_aula);
                        parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_RELATORIO, "Relatório de Aula de Reposicao");

                        List<Componentes.GenericModel.TO> sourceTO = apresentadorRelatorio.GetSource(FundacaoFisk.SGF.Utils.TipoRelatorioSGFEnum.AulasReposicaoSearch, parametrosPesquisa).ToList();

                        if (sourceTO != null && sourceTO.ToList<Componentes.GenericModel.TO>().Count > 0)
                            relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_AULA_REPOSICAO_VIEW, sourceTO, parametrosRelatorio);
                        else
                        {
                            //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
                            url = FundacaoFisk.SGF.ApresentacaoRelatorio.comum.UrlErro.ObterUrlErroRelatorio(url);
