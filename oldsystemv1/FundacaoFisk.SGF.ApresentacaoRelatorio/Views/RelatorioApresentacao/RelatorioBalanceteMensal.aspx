...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioBalanceteMensal.aspx");
            ...
            private void bindRelatorio(string url){
                ...
                int ano = 2014;
                int nivel = 0;
                int nivel_analisar = 0;
                int cdEscola = 0;
                bool mostrar_contas = true;
                bool renderizarRelatorio = true;
                bool conta_segura = true;
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if(parametrosHash[0].Equals("ano")) 
                            ano = int.Parse(parametrosHash[1]);
                        if(parametrosHash[0].Equals("nivel")) 
                            nivel = int.Parse(parametrosHash[1]);
                        if(parametrosHash[0].Equals("@nivel_analisar")){
                            nivel_analisar = int.Parse(parametrosHash[1]);
                            parametrosRelatorio.Add("nivel_plano", nivel_analisar);
                        }
                        if(parametrosHash[0].Equals("@mostrar_contas"))
                            mostrar_contas = bool.Parse(parametrosHash[1]);
                        if(parametrosHash[0].Equals("@conta_segura"))
                            conta_segura = bool.Parse(parametrosHash[1]);
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        sourceTO = apresentadorRelatorio.getSourceBalanceteMensal(cdEscola, mes, ano, nivel, nivel_analisar, mostrar_contas, conta_segura).ToList<Componentes.GenericModel.TO>();

                        // Inclui os parâmetros especializados do relatório:
                        parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_RELATORIO, "Relatório de Balancete Mensal");
                        parametrosRelatorio.Add("PPeriodo", (mes < 9 ? "0" + mes : mes.ToString())+"/"+ano);
                        
                        if (sourceTO != null && sourceTO.Count > 0)
                            relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_BALANCETE_MENSAL, sourceTO, parametrosRelatorio);
                        else
                        {
                            //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
                            url = FundacaoFisk.SGF.ApresentacaoRelatorio.comum.UrlErro.ObterUrlErroRelatorio(url);
