...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioFaixaEtaria.aspx");
            ...
            private void bindRelatorio(string url){
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if (parametrosHash[0].Equals("@idade"))
                            idade = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@idade_max"))
                            idade_max = int.Parse(parametrosHash[1]);
                        ...
                        //    parametrosRelatorio.Add(parametrosHash[0].Substring(1, parametrosHash[0].Length - 1), parametrosHash[1]);
                        if (!parametrosHash[0].StartsWith("@"))
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        parametrosRelatorio.Add("POpcao", tipo);
                        parametrosRelatorio.Add("PCdTurma", cd_turma);
                        parametrosRelatorio.Add("PNomeTurma", no_turma);
                        //Jáadicionado no Controller
                        //parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_RELATORIO, "Relatório de Faixa Etária");
                        Hashtable sourceHash = new Hashtable();

                        //List<st_RptFaixaEtaria_Result> dtReportData = apresentadorRelatorio.GetRtpFaixaEtaria(cd_escola, tipo, idade, idade_max, cd_turma);
                        DataTable dtReportData = apresentadorRelatorio.GetRtpFaixaEtariaDT(cd_escola, tipo, idade, idade_max, cd_turma);
                        ...
                        if (dtReportData != null && dtReportData.Rows != null && dtReportData.Rows.Count > 0)
                        {
                            try
                            {
                                sourceHash["DataFaixaEtaria"] = dtReportData;
                                relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_FAIXA_ETARIA, sourceHash, parametrosRelatorio);
                            }
                            catch (Exception e)
                            {
                                Console.WriteLine(e);
