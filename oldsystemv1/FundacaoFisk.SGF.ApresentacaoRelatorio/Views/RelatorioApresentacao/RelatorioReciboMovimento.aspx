            ...
            private void bindRelatorio(string url){
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if (parametrosHash[0].Equals("@cd_baixa_titulo"))
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        FundacaoFisk.SGF.Web.Services.Financeiro.Model.Recibo recibo = apresentadorRelatorio.GetSourceReciboBaixa(cdBaixaTitulo, cdEscola);
                        List<FundacaoFisk.SGF.GenericModel.ItemMovimento> itensMovimento = apresentadorRelatorio.getItensMovimentoRecibo(cd_movimento, cdEscola);
                        ...
                        sourceHash["DataSetReportItens"] = itensMovimento;
                        if (sourceTO != null && sourceTO.ToList<Componentes.GenericModel.TO>().Count > 0)
                            relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_RECIBO_MOVIMENTO, sourceHash, parametrosRelatorio);
                        else
                        {
                            //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
                            url = FundacaoFisk.SGF.ApresentacaoRelatorio.comum.UrlErro.ObterUrlErroRelatorio(url);
