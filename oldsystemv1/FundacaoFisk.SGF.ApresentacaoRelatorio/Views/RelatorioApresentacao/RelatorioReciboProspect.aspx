...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioReciboProspect.aspx");
            ...
            public void Page_Load(object sender, System.EventArgs e)
            {
                ...
                catch(FundacaoFisk.SGF.Web.Services.Secretaria.Business.SecretariaBusinessException exe){
                    if(exe.tipoErro == FundacaoFisk.SGF.Web.Services.Secretaria.Business.SecretariaBusinessException.TipoErro.ERRO_RECIBO_SEM_BAIXA) 
            ...
            private void RedirecionaAplicacaoPrincipalNegocio(string url, Exception exe)
            {
                try
                {
                    string stackTrace = HttpUtility.UrlEncode(exe.Message);
                    ...
                    logger.Warn(FundacaoFisk.SGF.Utils.Messages.Messages.msgEmissaoRelatorioError, exe);
                    Response.Redirect(url + "?msgErro=" + stackTrace + "&stackTrace=");
                }
                catch (Exception e)
                {
            ...
            private void bindRelatorio(string url){
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if (parametrosHash[0].Equals("@pCodProspect"))
                            cdProspect = int.Parse(parametrosHash[1]);
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        FundacaoFisk.SGF.Web.Services.Financeiro.Model.Recibo recibo = apresentadorRelatorio.getSourceReciboBaixaProspect(cdProspect, cdEscola);
                        ...
                        if (sourceTO != null && sourceTO.ToList<Componentes.GenericModel.TO>().Count > 0)
                            relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_RECIBO, sourceTO, parametrosRelatorio);
                        else
                        {
                            //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
                            url = FundacaoFisk.SGF.ApresentacaoRelatorio.comum.UrlErro.ObterUrlErroRelatorio(url);
