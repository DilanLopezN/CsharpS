...
<body>
    <div>
     <script runat="server">
         ...
         private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioMediaAlunos.aspx");
         ...
         private void bindRelatorio(string url) {
             ...
             if (parms != null)
             {
                 ...
                 for (int i = 0; i < parametrosGet.Length; i++)
                 {
                     ...
                     if (parametrosHash[0].Equals("@pesOpcao"))
                         pesOpcao = int.Parse(parametrosHash[1]);
                     if (parametrosHash[0].Equals("@pesTipoAluno"))
                         pesTipoAluno = int.Parse(parametrosHash[1]);
                     if (parametrosHash[0].Equals("vl_media") && !parametrosHash[1].IsEmpty())
                         vl_media = decimal.Parse(parametrosHash[1]);
                     if (parametrosHash[0].Equals("@dtInicial") && parametrosHash[1] != "")
                         dtInicial = DateTime.Parse(parametrosHash[1]);
                     if (parametrosHash[0].Equals("@dtFinal") && parametrosHash[1] != "")
                         dtFinal = DateTime.Parse(parametrosHash[1]);
                     if (parametrosHash[0].Equals("mostrarAvaliacao") )
                         mostrarAvaliacao = bool.Parse(parametrosHash[1]);
                     if (!parametrosHash[0].StartsWith("@"))
                     {
                         if (parametrosHash[0].Equals("vl_media"))
                         {
                             if(parametrosHash[1].IsEmpty())
                                 parametrosRelatorio.Add(parametrosHash[0], "Todas");
                             else
                                 parametrosRelatorio.Add(parametrosHash[0], string.Format("{0:0,0.00}", parametrosHash[1]));
                         }
                         else if (parametrosHash[0].Equals("no_turma") && parametrosHash[1].IsEmpty())
                             parametrosRelatorio.Add(parametrosHash[0], "Todas");
                         else
             ...
             if (renderizarRelatorio)
             {
                 ...
                 //List<Componentes.GenericModel.TO> mediaAlunos = apresentadorRelatorio.getRptMediaAlunos(cd_escola, cd_turma, tipoTurma, cdCurso, cdProduto, pesOpcao, pesTipoAluno, vl_media, dtInicial, dtFinal).ToList<Componentes.GenericModel.TO>();
                 //IEnumerable<FundacaoFisk.SGF.GenericModel.sp_getRptMediaAlunos_Result> mediaAlunos = apresentadorRelatorio.getRptMediaAlunos(cd_escola, cd_turma, tipoTurma, cdCurso, cdProduto, pesOpcao, pesTipoAluno, vl_media, dtInicial, dtFinal);
                 System.Data.DataTable dtReportData = apresentadorRelatorio.getRptMediaAlunos(cd_escola, cd_turma, tipoTurma, cdCurso, cdProduto, pesOpcao, pesTipoAluno, vl_media, dtInicial, dtFinal);
                 if ((dtInicial != null && dtInicial != DateTime.MinValue) || (dtFinal != null && dtFinal != DateTime.MinValue))
                     parametrosRelatorio.Add("PPeriodo", "Período de " + String.Format("{0:dd/MM/yyyy}", dtInicial) + " à " + String.Format("{0:dd/MM/yyyy}", dtFinal));
                 parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_RELATORIO, "Relatório de Turmas Melhores Notas");
                 parametrosRelatorio.Add("DataBase", dataBase);
                 //parametrosRelatorio.Add("mostrarAvaliacao", mostrarAvaliacao);
                 //if (mediaAlunos != null && mediaAlunos.Count() > 0)
                 if (dtReportData != null && dtReportData.Rows != null && dtReportData.Rows.Count > 0)
                 {
                     sourceHash["DataSetReport"] = dtReportData;  //mediaAlunos
                     relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_MEDIA_ALUNOS, sourceHash, parametrosRelatorio);
                 }
                 else
                 {
                     //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
