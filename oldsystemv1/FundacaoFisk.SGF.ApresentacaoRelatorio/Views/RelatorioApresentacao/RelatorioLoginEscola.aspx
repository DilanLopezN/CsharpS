...
<body>
    <div>
     <script runat="server">
         ...
         private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioLoginEscola.aspx");
         ...
         private void bindRelatorio(string url, ref bool abortou_requisicao)
         {
             ...
             if (parms != null)
             {
                 ...
                 for (int i = 0; i < parametrosGet.Length; i++)
                 {
                     ...
                     if (parametrosHash[0].Equals("@dtAnalise") && parametrosHash[1] != "")
                         dtAnalise = (parametrosHash[1] + "");
                     if (parametrosHash[0].Equals("@hhAnalise") && parametrosHash[1] != "")
                         hhAnalise = (parametrosHash[1] + "");
                     if (parametrosHash[0].Equals("@idLogin"))
                         idLogin = bool.Parse(parametrosHash[1]);
                     if (parametrosHash[0].Equals("@idMatricula"))
                         idMatricula = byte.Parse(parametrosHash[1]);
             ...
             DateTime dt_analise = DateTime.Parse(dtAnalise);
             TimeSpan hora = TimeSpan.Parse(hhAnalise);
             dt_analise = dt_analise.AddHours(hora.Hours);
             dt_analise = dt_analise.AddMinutes(hora.Minutes);
             ...
             if (renderizarRelatorio)
             {
                 ...
                 string nomeRelatorio = "RptLoginEscola.rdlc";
                 string tituloRel = "Relatório de Uso SGF, Data Análise: " + dtAnalise + " " + hhAnalise;
                 ...
                 parametrosRelatorio.Add("IdLogin", idLogin);
                 parametrosRelatorio.Add("IdMatricula", idMatricula);

                 Hashtable sourceHash = new Hashtable();

                 DataTable dtReportData = apresentadorRelatorio.getLoginEscola(dt_analise, idLogin, idMatricula);
                 ...
                 if (dtReportData != null && dtReportData.Rows != null && dtReportData.Rows.Count > 0)
                 {
                     try
                     {
                         ...
                         relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_USO_SGF, sourceHash, parametrosRelatorio);
                     }
                     catch (Exception e)
                     {
                         Console.WriteLine(e);
