...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioDinamico.aspx");
            ...
            public void Page_Load(object sender, System.EventArgs e)
            {
                ...
                try {
                    logger.Debug("IsPostBack R");
                    if(!IsPostBack){
                        logger.Debug("Url-->"+url);
                        bindRelatorio(url);
                    }
                    logger.Debug("fim - IsPostBack R");
                }
                catch(Exception exe) {
            ...
            private void RedirecionaAplicacaoPrincipal(string url, Exception exe)
            {
                try
                {
                    ...
                    if (exe.Message != Componentes.Utils.Messages.Messages.msgSessaoExpirada)
                        logger.Debug(FundacaoFisk.SGF.Utils.Messages.Messages.msgEmissaoRelatorioError, exe);
                ...
                {
                    logger.Debug(FundacaoFisk.SGF.Utils.Messages.Messages.msgEmissaoRelatorioError + " url: " + url, e);
                }
            }
            ...
            private void bindRelatorio(string url)
            {
                ...
                ConfiguradorRelatorio.Orientation orientation = ConfiguradorRelatorio.Orientation.PORTRAIT;
                ...
                if(parms != null) {
                    ...
                    FundacaoFisk.SGF.Utils.TipoRelatorioSGFEnum tipoRelatorioDinamico = FundacaoFisk.SGF.Utils.TipoRelatorioSGFEnum.EscolaridadeSearch;
                    logger.Debug("parametrosGet R");
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        string[] parametrosHash = parametrosGet[i].Split('=');

                        if(!Componentes.Utils.ReportParameter.PARAMETRO_TIPO_RELATORIO.Equals(parametrosHash[0])) {
                            if ("@orientation".Equals(parametrosHash[0]))
                                if (parametrosHash[1] != null && "LANDSCAPE".Equals(parametrosHash[1].ToUpper()))
                                    orientation = ConfiguradorRelatorio.Orientation.LANDSCAPE;
                            if (parametrosHash[0].Equals("@cd_escola_combo"))
                                cdEscolaCombo = int.Parse(parametrosHash[1]);
                            ...
                            tipoRelatorioDinamico = (FundacaoFisk.SGF.Utils.TipoRelatorioSGFEnum) Enum.Parse(typeof(FundacaoFisk.SGF.Utils.TipoRelatorioSGFEnum), parametrosHash[1]);
                    }
                    try {
                        logger.Debug("PDataHoraAtual R");
                        ...
                        /*if(Session["NomeEscolaSelecionada"] != null)
                            parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_EMPRESA, Session["NomeEscolaSelecionada"] + "");
                        if(Session["loginUsuario"] != null)
                            parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_USUARIO, Session["loginUsuario"] + "");
                        parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_DATA_HORA_ATUAL, DateTime.Now);*/
                        logger.Debug("caminho relatorio  - " + ConfigurationManager.AppSettings["caminhoRelatorio"]);
                        ...
                        if (renderizarRelatorio)
                        {
                            ...
                            if (cdEscolaCombo > 0)
                            {
                                string nomeEscolaCombo = apresentadorRelatorio.GetSourceNomeEscolaComboReportView(cdEscolaCombo);
                                if (nomeEscolaCombo != null)
                                {
                                    parametrosRelatorio["PEmpresa"] = nomeEscolaCombo;
                                }
                            }

                            if(sourceTO != null && sourceTO.Count > 0)
                                relatorio.ExibirRelatorioDinamico(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_DINAMICO, sourceTO, parametrosRelatorio, orientation);
                            else {
                                //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
                                url = FundacaoFisk.SGF.ApresentacaoRelatorio.comum.UrlErro.ObterUrlErroRelatorio(url);
                        ...
                        throw new Exception("Tela: " + tipoRelatorioDinamico, exe);
                    }
                }
                else
        ...
        <form id="Form1" runat="server" style="width:100%; height:100%;">
            ...
            <cc1:VisualizadorRelatorio id="relatorio" runat="server" height="100%" width="100%" Font-Names="Verdana" DocumentMapCollapsed="false"  ShowFindControls="true"
                ...
                OnBookmarkNavigation="relatorio_BookmarkNavigation" OnPageNavigation="relatorio_PageNavigation"></cc1:VisualizadorRelatorio>
        </form>
    </div>
</body>
</html>