     ...
     <script runat="server">
            ...
            private void RedirecionaAplicacaoPrincipal(bool abortou_requisicao, string url, Exception exe)
            {
                try
                {
                    ...
                    string stackTrace = HttpUtility.UrlEncode(exe.Message + exe.InnerException);
            ...
            { 
                ...
                if (parms != null)
                {
                    ...
                    for (int i = 0; i < parametrosGet.Length; i++)
                    {
                        ...
                        if (parametrosHash[0].Equals("@tipoLiquidacao"))
                            tipoLiquidacao = bool.Parse(parametrosHash[1]);
                ...
                // Adicionando o periodo 
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        IEnumerable<FundacaoFisk.SGF.Web.Services.Financeiro.Model.SaldoFinanceiro> contasCorrentes = apresentadorRelatorio.rptSaldoFinanceiro(cdEscola, dataBase, tipo, tipoLiquidacao).ToList();
                        var nomeRelatorio = FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_SALDO_FINANCEIRO;
                        parametrosRelatorio.Add("DataBase", dataBase);
                        parametrosRelatorio.Add("PGradeSaldoLiquidacao", tipoLiquidacao);
                        parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_RELATORIO, "Listagem Saldos Financeiros");
                        parametrosRelatorio.Add("PPeriodo", "Período: " + String.Format("{0:dd/MM/yyyy}", dataBase));
                        var listaSaldosBancosLiquidacao = contasCorrentes.GroupBy(x => new { x.id_tipo, x.banco, x.tipo, x.nm_tipo }).Select(g => new FundacaoFisk.SGF.Web.Services.Financeiro.Model.SaldoFinanceiro
                        {
                        ...
                        var listaSaldosLiquidacao = contasCorrentes.GroupBy(x => new { x.nm_tipo, x.tipo, x.id_tipo }).Select(g => new FundacaoFisk.SGF.Web.Services.Financeiro.Model.SaldoFinanceiro
                        {
                            nm_tipo = g.Key.nm_tipo,
                            tipo = g.Key.tipo,
                            //tipo = g.Key.tipo,
                        ...
                        sourceHash["DataSetReport"] = listaSaldosBancosLiquidacao;
                        sourceHash["DataSetReportLiquidacao"] = listaSaldosLiquidacao;

                        if (contasCorrentes != null && contasCorrentes.Count() > 0)
                        {
