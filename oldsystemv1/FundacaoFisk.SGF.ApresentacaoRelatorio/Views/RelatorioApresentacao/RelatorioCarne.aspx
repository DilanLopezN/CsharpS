...
<body>
    <div>
     <script runat="server">
            ...
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioCarne.aspx");
            ...
            private void bindRelatorio(string url) { 
                ...
                if (parms != null)
                {
                    ...
                    for (int i = 0; i < parametrosGet.Length; i++)
                    {
                        ...
                        if (parametrosHash[0].Equals("@cdContrato"))
                            cdContrato = int.Parse(parametrosHash[1]);
                ...
                if (renderizarRelatorio)
                {
                    ...
                    IEnumerable<FundacaoFisk.SGF.Web.Services.Financeiro.Model.CarneUI> carnes = apresentadorRelatorio.getCarnePorContrato(cdContrato, cdEscola, contaSegura).ToList();


                    var nomeRelatorio = FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_CARNE;
                    ...
                    if (sourceTO != null && sourceTO.Count > 0)
                    {
                        ...
                        relatorio.LocalReport.SubreportProcessing += new Microsoft.Reporting.WebForms.SubreportProcessingEventHandler(renderizaSubRptCarne);
                    }
                    else
                    {
                        //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
            ...
            private void renderizaSubRptCarne(object sender, Microsoft.Reporting.WebForms.SubreportProcessingEventArgs e)
            {
                try
                {
                    ...
                    //decimal vl_multa = Convert.ToDecimal(e.Parameters["vl_multa"].Values[0]);
                    //decimal val_acr = Convert.ToDecimal(e.Parameters["val_acr"].Values[0]);
                        
                    FundacaoFisk.SGF.Web.Services.ApresentadorRelatorio.RelatorioController apresentadorRelatorio = new FundacaoFisk.SGF.Web.Services.ApresentadorRelatorio.RelatorioController();
                    e.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource("DataSetSubReport",
                        apresentadorRelatorio.getTituloCarnePorContratoSubReport(cd_titulo, cd_pessoa_empresa, conta_segura)));
                }
                catch (Exception exe)
                {
                    logger.Error(FundacaoFisk.SGF.Utils.Messages.Messages.msgEmissaoRelatorioError, exe);
            ...
            protected void relatorio_Load(object sender, EventArgs e)
            {
                ...
                lista.Add("Excel");
                lista.Add("EXCELOPENXML");
                //lista.Add("PDF");
                lista.Add("WORD");
                lista.Add("WORDOPENXML");

                foreach (var exportOption in lista)
                {
                    RenderingExtension extension = relatorio.LocalReport.ListRenderingExtensions().ToList().Find(x => x.Name.Equals(exportOption, StringComparison.CurrentCultureIgnoreCase));
                    if (extension != null)
                    {
