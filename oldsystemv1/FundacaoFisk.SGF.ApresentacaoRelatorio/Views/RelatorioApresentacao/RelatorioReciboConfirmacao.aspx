...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioReciboConfirmacao.aspx");
            Hashtable parametrosSubRelatorio = new Hashtable();
            ...
            private void bindRelatorio(string url){
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if (parametrosHash[0].Equals("@pCdContrato"))
                    ...
                    if (renderizarRelatorio)
                    {
                        try
                        {
                            ...
                            if (cdOrigemTitulo == (int)FundacaoFisk.SGF.GenericModel.Titulo.OrigemTitulo.CONTRATO)
                            {
                                ...
                                recibo = apresentadorRelatorio.getReciboConfirmacaoByContrato(cd_contrato, cdEscola);
                                path = FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_RECIBO_CONFIRMACAO;
                                parametrosSubRelatorio.Add("PCdContrato", cd_contrato);
                                parametrosSubRelatorio.Add("PCdEscola", cdEscola);
                            }
                            else
                            {
                                parametrosRelatorio.Remove("PNomeUsuario");
                                recibo = apresentadorRelatorio.getReciboConfirmacaoByMovimento(cd_contrato, cdEscola);
                                path = FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_RECIBO_CONFIRMACAO;
                                parametrosSubRelatorio.Add("PCdContrato", cd_contrato);
                                parametrosSubRelatorio.Add("PCdEscola", cdEscola);
                            }
                            ...
                            if (sourceTO != null && sourceTO.ToList<Componentes.GenericModel.TO>().Count > 0)
                            {
                                ...
                                if (cdOrigemTitulo == (int)FundacaoFisk.SGF.GenericModel.Titulo.OrigemTitulo.CONTRATO)
                                {
                                    relatorio.LocalReport.SubreportProcessing += new Microsoft.Reporting.WebForms.SubreportProcessingEventHandler(renderizaSubRelatorioParcelasReciboConfirmacao);
                                }
                                else
                                {
                                    relatorio.LocalReport.SubreportProcessing += new Microsoft.Reporting.WebForms.SubreportProcessingEventHandler(renderizaSubRelatorioParcelasReciboConfirmacaoMovimento);
                                }
                            }
                            else
                            {
            ...
            private void renderizaSubRelatorioParcelasReciboConfirmacao(object sender, Microsoft.Reporting.WebForms.SubreportProcessingEventArgs e)
            {
                try
                {
                    int cd_contrato = Convert.ToInt32(parametrosSubRelatorio["PCdContrato"]);
                    int cd_escola = Convert.ToInt32(parametrosSubRelatorio["PCdEscola"]);
                    FundacaoFisk.SGF.Web.Services.ApresentadorRelatorio.RelatorioController apresentadorRelatorio = new FundacaoFisk.SGF.Web.Services.ApresentadorRelatorio.RelatorioController();

                    e.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource("DataSetSubReportReciboConfirmacao",
                       apresentadorRelatorio.getParcelasReciboConfirmacaoByContrato(cd_contrato, cd_escola)));
                }
                catch (Exception exe)
                {
                    logger.Error(FundacaoFisk.SGF.Utils.Messages.Messages.msgEmissaoRelatorioError, exe);
            ...
            private void renderizaSubRelatorioParcelasReciboConfirmacaoMovimento(object sender, Microsoft.Reporting.WebForms.SubreportProcessingEventArgs e)
            {
                try
                {
                    int cd_movimento = Convert.ToInt32(parametrosSubRelatorio["PCdContrato"]);
                    int cd_escola = Convert.ToInt32(parametrosSubRelatorio["PCdEscola"]);
                    FundacaoFisk.SGF.Web.Services.ApresentadorRelatorio.RelatorioController apresentadorRelatorio = new FundacaoFisk.SGF.Web.Services.ApresentadorRelatorio.RelatorioController();

                    e.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource("DataSetSubReportReciboConfirmacao",
                        apresentadorRelatorio.getParcelasReciboConfirmacaoByMovimento(cd_movimento, cd_escola)));
                }
                catch (Exception exe)
                {
                    logger.Error(FundacaoFisk.SGF.Utils.Messages.Messages.msgEmissaoRelatorioError, exe);
