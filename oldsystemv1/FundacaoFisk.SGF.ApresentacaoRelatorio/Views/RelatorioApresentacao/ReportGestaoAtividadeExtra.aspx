...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("ReportGestaoAtividadeExtra.aspx");

            public int cdAluno;//Parametro Global para o subreport
            public int cdEscolaAluno;//Parametro Global para o subreport
            public byte idParticipacao;//Parametro Global para o subreport
            public byte idLancada;//Parametro Global para o subreport
            ...
            private void bindRelatorio(string url){
                ...
                int cdAtividadeExtra = 0;
                DateTime? dta_ini = null;
                DateTime? dta_fim = null;
                int cd_produto = 0;
                int cd_curso = 0;
                int cd_funcionario = 0;
                int cd_aluno = 0;
                byte id_participacao = 0;
                byte id_lancada = 0;
                int cdEscola = 0;
                bool esconde_obs = false;
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if (parametrosHash[0].Equals("@cd_atividade_extra"))
                            cdAtividadeExtra = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@cd_escola"))
                            cdEscola = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@dta_ini"))
                            dta_ini = DateTime.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@dta_fim"))
                            dta_fim = !String.IsNullOrEmpty(parametrosHash[1]) ? DateTime.Parse(parametrosHash[1]) : (DateTime?)null;
                        if (parametrosHash[0].Equals("@cd_produto"))
                            cd_produto = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@cd_curso"))
                            cd_curso = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@cd_funcionario"))
                            cd_funcionario = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@cd_aluno"))
                            cd_aluno = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@cd_aluno"))
                            cd_aluno = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@id_participacao"))
                            id_participacao = Byte.Parse(parametrosHash[1], System.Globalization.NumberStyles.None, System.Globalization.CultureInfo.CreateSpecificCulture("pt-BR"));
                        if (parametrosHash[0].Equals("@esconde_obs"))
                            esconde_obs = bool.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@id_lancada"))
                            id_lancada = Byte.Parse(parametrosHash[1], System.Globalization.NumberStyles.None, System.Globalization.CultureInfo.CreateSpecificCulture("pt-BR"));
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        List<FundacaoFisk.SGF.GenericModel.sp_RptAtividadeExtra_Result> atividadeExtra = apresentadorRelatorio.GetSourceAtividadeExtra(cdEscola, dta_ini, dta_fim, cd_produto, cd_curso, cd_funcionario, cd_aluno, id_participacao, id_lancada);
                        List<Componentes.GenericModel.TO> sourceTO = null;
                        if (atividadeExtra.Count() > 0)
                        {

                            atividadeExtra = cdAtividadeExtra > 0 ? atividadeExtra.Where(x => x.no_tipo_atividade_extra.Equals(
                                                apresentadorRelatorio.GetSourceTipoAtividadeExtraAluno(cdAtividadeExtra, cdEscola).ToList().FirstOrDefault().no_tipo_atividade_extra)).ToList() : atividadeExtra;

                            sourceTO = atividadeExtra.ToList<Componentes.GenericModel.TO>();
                        }

                        parametrosRelatorio.Add("PDataIni", dta_ini);
                        parametrosRelatorio.Add("PDataFim", dta_fim);
                        parametrosRelatorio.Add("PEsconder", esconde_obs);
                        Hashtable sourceHash = new Hashtable();

                        if (sourceTO != null && sourceTO.ToList<Componentes.GenericModel.TO>().Count > 0)
                        {
                            this.cdAluno = cd_aluno;
                            this.idParticipacao = id_participacao;
                            this.idLancada = id_lancada;
                            this.cdEscolaAluno = cdEscola;
                            try
                            {
                                sourceHash["Atividades"] = atividadeExtra;
                                relatorio.LocalReport.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource("Atividades", sourceTO));
                                relatorio.LocalReport.SubreportProcessing += new Microsoft.Reporting.WebForms.SubreportProcessingEventHandler(AlunoSubReportProcessing);
                                relatorio.LocalReport.Refresh();
                                relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_GESTAO_ATIVIDADE_EXTRA, sourceHash, parametrosRelatorio);
                            }
                            catch (Exception e)
                            {
                                Console.WriteLine(e);
            ...
            private void AlunoSubReportProcessing(object sender, Microsoft.Reporting.WebForms.SubreportProcessingEventArgs e)
            {

                int cd_atividade = int.Parse(e.Parameters["cd_atividade_extra"].Values[0].ToString());
                FundacaoFisk.SGF.Web.Services.ApresentadorRelatorio.RelatorioController apresentadorRelatorio = new FundacaoFisk.SGF.Web.Services.ApresentadorRelatorio.RelatorioController();

                IEnumerable<FundacaoFisk.SGF.GenericModel.sp_RptAtividadeExtraAluno_Result> dataTableAtividadeExtraAluno = apresentadorRelatorio.GetSourceAtividadeExtraAluno(cd_atividade, this.cdAluno, this.idParticipacao, this.idLancada, this.cdEscolaAluno).ToList();
                e.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource("Alunos", dataTableAtividadeExtraAluno));
            }
