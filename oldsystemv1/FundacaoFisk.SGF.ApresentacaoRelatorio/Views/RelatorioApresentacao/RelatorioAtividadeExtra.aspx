...
<body>
    <div style="Width:100%;height:100%;">
        <script runat="server">
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioAtividadeExtra.aspx");
            ...
            private void bindRelatorio(string url){
                ...
                if(parms != null) {
                    ...
                    for(int i = 0; i < parametrosGet.Length; i++) {
                        ...
                        if(parametrosHash[0].Equals("@cdAtividadeExtra"))
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        FundacaoFisk.SGF.GenericModel.AtividadeExtra atividadeExtra = apresentadorRelatorio.GetSourceAtividadeExtraReportView(cdAtividadeExtra);
                        ...
                        // Inclui os parâmetros especializados do relatório de atividade extra:
                        parametrosRelatorio.Add("PTipoAtividade", atividadeExtra.TipoAtividadeExtra.no_tipo_atividade_extra);
                        parametrosRelatorio.Add("PData", String.Format("{0:dd/MM/yyyy}", atividadeExtra.dt_atividade_extra));
                        parametrosRelatorio.Add("PResponsavel", atividadeExtra.Funcionario.FuncionarioPessoaFisica != null ? atividadeExtra.Funcionario.FuncionarioPessoaFisica.no_pessoa : "");
                        parametrosRelatorio.Add("PHorario", atividadeExtra.tempo);
                        parametrosRelatorio.Add("PVagas", atividadeExtra.nm_vagas.HasValue ? atividadeExtra.nm_vagas : null);
                        parametrosRelatorio.Add("PSala", atividadeExtra.AtividadeSala != null ? atividadeExtra.AtividadeSala.no_sala : "");
                        parametrosRelatorio.Add("PObservacao", atividadeExtra.tx_obs_atividade);
                        parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_RELATORIO, "Relatório de Atividade Extra");

                        List<Componentes.GenericModel.TO> sourceTO = apresentadorRelatorio.GetSource(FundacaoFisk.SGF.Utils.TipoRelatorioSGFEnum.AtividadeExtraSearch, parametrosPesquisa).ToList();

                        if (sourceTO != null && sourceTO.ToList<Componentes.GenericModel.TO>().Count > 0)
                            relatorio.ExibirRelatorioEspecializado(ConfigurationManager.AppSettings["caminhoRelatorio"] + "", FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_ATIVIDADE_EXTRA, sourceTO, parametrosRelatorio);
                        else
                        {
                            //url = Request.QueryString["enderecoWeb"] != null ? HttpUtility.UrlDecode(Request.QueryString["enderecoWeb"], System.Text.Encoding.UTF8) + "/Erro" : ConfigurationManager.AppSettings["enderecoRetornoErro"];
                            url = FundacaoFisk.SGF.ApresentacaoRelatorio.comum.UrlErro.ObterUrlErroRelatorio(url);
