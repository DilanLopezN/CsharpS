...
<body>
    <div>
     <script runat="server">
            ...
            private static readonly log4net.ILog logger = log4net.LogManager.GetLogger("RelatorioContaCorrete.aspx");
            ...
            { 
                ...
                if (parms != null)
                {
                    ...
                    for (int i = 0; i < parametrosGet.Length; i++)
                    {
                        ...
                        if (parametrosHash[0].Equals("@pLocalMovto"))
                            cdLocalMovto = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("PEmpresa"))
                            escola = parametrosHash[1];
                        if (parametrosHash[0].Equals("PNomeUsuario"))
                            usuario = parametrosHash[1];
                        if (parametrosHash[0].Equals("@tipoLiquidacao"))
                            tipoLiquidacao = int.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@desTipoLiquidacao"))
                            desTipoLiquidacao = parametrosHash[1];
                        if (parametrosHash[0].Equals("@contaSegura"))
                            contaSegura = bool.Parse(parametrosHash[1]);
                        if (parametrosHash[0].Equals("@isMaster"))
                            isMaster = bool.Parse(parametrosHash[1]);
                ...
                // Adicionando o periodo 
                    ...
                    if (renderizarRelatorio)
                    {
                        ...
                        IEnumerable<FundacaoFisk.SGF.Web.Services.Financeiro.Model.ContaCorrenteUI> contasCorrentes =  apresentadorRelatorio.getReportContaCorrente(cdEscola, pDtaI.Value, pDtaF.Value, cdLocalMovto, tipoLiquidacao, contaSegura, isMaster).ToList();
                        
                     
                        var nomeRelatorio = FundacaoFisk.SGF.Utils.ReportParameter.RELATORIO_CONTA_CORRENTE;
                        
                        string tituloRel = "Conta Corrente";
                        ...
                        parametrosRelatorio.Add("PDtaI", pDtaI);
                        parametrosRelatorio.Add("PDtaF", pDtaF);
                        parametrosRelatorio.Add("PLocalMovto", contasCorrentes.ToList()[0].des_local_movto);
                        parametrosRelatorio.Add("PEscola", contasCorrentes.ToList()[0].cd_pessoa_escola);
                        parametrosRelatorio.Add("PLocal", contasCorrentes.ToList()[0].cd_local_movimento);
                        parametrosRelatorio.Add("PTipo", desTipoLiquidacao);

                        parametrosRelatorio.Add(Componentes.Utils.ReportParameter.PARAMETRO_NOME_RELATORIO, tituloRel);

                        sourceTO = contasCorrentes.ToList<Componentes.GenericModel.TO>();
