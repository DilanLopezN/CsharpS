...
namespace Simjob.Framework.Services.Api.Controllers
{
  public class PessoaController : BaseController
  {
      ...
      if (source != null && source.Active != null && source.Active == true)
      {
        ...
        if (enderecoExists != null)
        {
          ...
          if (cep != null)
          {
            ...
            var cd_tipo_endereco = enderecoExists["cd_tipo_endereco"]?.ToString() ?? "";
            var cd_tipo_logradouro = enderecoExists["cd_tipo_logradouro"]?.ToString() ?? "";

            var infosCep = await BuscarCEP(cep.ToString(), numero, complemento, cd_tipo_endereco, cd_tipo_logradouro, source);
            if (infosCep != null)
            {
      ...
      if (command.pessoa.nm_natureza_pessoa == 1)
      {
        if (command.pessoa.nm_cpf != null && !string.IsNullOrEmpty(command.pessoa.nm_cpf))
        {
          ...
          if (cpfExist != null)
          {
            ...
            var filtrosPessoa = new List<(string campo, object valor)> { new("cd_pessoa", cd_pessoa_cpf) };
            var pessoaExists = await SQLServerService.GetFirstByFields(source, "T_PESSOA", filtrosPessoa);

            if (pessoaExists != null)
            {
              var isDelete = pessoaExists.ContainsKey("is_delete") && pessoaExists["is_delete"] != null
                  ? Convert.ToInt32(pessoaExists["is_delete"])
                  : 0;

              // Apenas bloqueia se está ativo (is_delete = 0)
              if (isDelete == 0)
              {
        ...
        if (!string.IsNullOrEmpty(model.nm_cpf))
        {
          ...
          if (cpfExist != null)
          {
            ...
            if (pessoaExists != null)
            {
              ...
              if (isDelete == 0)
              {
                return (false, $"nm_cpf ja existente({model.nm_cpf})");
              }
            }
          }
        }
    ...
    private async Task<dynamic?> BuscarCEP(string cep, string numero, string complemento, string? cd_tipo_endereco, string? cd_tipo_logradouro, Source source)
    {
