    ...
    public async Task<IActionResult> PutPipeline(UpdatePipelineModel model)
    {
      ...
      if (source != null && source.Active != null && source.Active == true)
      {
        ...
        if (model.Pessoa != null)
        {
          ...
          var cd_email = emailExists != null ? emailExists["cd_telefone"] : null; // CORRIGIDO
          ...
          var cd_telefone = telefoneExists != null ? telefoneExists["cd_telefone"] : null; // CORRIGIDO
          ...
          var cd_celular = celularexists != null ? celularexists["cd_telefone"] : null; // CORRIGIDO
          ...
          if (model.Pessoa.EnderecoPessoaPipeLine != null)
          {
            ...
            if (enderecoExists != null)
            {
              ...
              var cd_endereco = enderecoExists["cd_endereco"];
              var t_endereco_update = await SQLServerService.Update("T_ENDERECO", enderecoDict, source, "cd_endereco", cd_endereco);
              if (!t_endereco_update.success) return BadRequest(t_endereco_update.error);
            }
            else
            {
              var enderecoDict = new Dictionary<string, object>
